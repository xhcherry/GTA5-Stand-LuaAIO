...
local config = {
    ...
    Auto_translate = false,--默认自动翻译器状态
    translateSelf = false,--默认翻译自己
    Auto_translate_self = false,--默认自动翻译自己
...
for i = 1, #LangPairs do
    ...
    local new_name = {}
    new_name[1] = i
    new_name[2] = LangPairs[i].name
    LangNameList[i] = new_name
end
...
function createGoogleTranslateCall(config)
    return function(text, targetLang, onSuccess)
        ...
        async_http.init("translate.googleapis.com", "/translate_a/t?" .. tableToUrlParams(params),function(body, header_fields, status_code)
                if status_code == 200 and body ~= nil then
            ...
            if not config.translateSelf and (sender == players.user()) then
                ...
                translateTextCB(text, config.targetLanguageIncoming, config.translationMethodIncoming, config, function(translation, sourceLang)
                    ...
                    if (translatedMsgLocation == 5) then--通知
                        notification("~y~~bold~"..senderName .. " : " .. resultText, math.random(0, 200), "翻译")
                    end
                end)
            end
        end
...
local home_list = menu.list(chat_transl, "国内线路", {}, "")
local google_list = menu.list(chat_transl, "谷歌线路", {}, "")

menu.toggle(home_list, "全自动翻译", {},"自动识别语言并翻译\n仅支持中英互译,遇到其他语言可能乱码", function(on)
    config.Auto_translate = on
end)
menu.toggle(home_list, "翻译自我", {},"选择是否要翻译自己发送的消息", function(on)
    config.Auto_translate_self = on
end)
menu.toggle_loop(home_list, "发送翻译文本", {}, "仅使用T or Y即可键入文本\n当使用此选项时你应该首选关闭翻译自我", function()
    if PAD.IS_CONTROL_PRESSED(0,245) or PAD.IS_CONTROL_PRESSED(0,246) then
        local label = util.register_label("输入文本")
        local input = get_input_from_screen_keyboard(label, 254, "")
        if input == "" then return end
        async_http.init("api.gumengya.com","/Api/Translate?format=json&text=" ..url_encode(input) .. "&from=auto&to=auto",function(body, header_fields, status_code)
            if status_code == 200 and body ~= nil then
                local tab = StrToTable(body)
                if tonumber(tab.code) == 200 then
                    if tab.data.from ~= tab.data.to then--判断源语言是否=目标语言
                        local senderName = players.get_name(PLAYER.PLAYER_ID())
                        botSend = true
                        chat.send_message(tab.data.result, false, true, true)
                    end
                end
            else
                util.toast("翻译出错啦")
...
chat.on_message(function(sender, reserved, text, team_chat, networked, is_auto)
    if config.Auto_translate then
        if not botSend then
            if not config.Auto_translate_self and (sender == players.user()) then
                return
            else
                    ...
                    if status_code == 200 and body ~= nil then
                        ...
                        if tonumber(tab.code) == 200 then
                            if tab.data.from ~= tab.data.to then--判断源语言是否=目标语言
                                ...
                                if (translatedMsgLocation == 5) then--通知
                                    notification("~y~~bold~"..senderName .. " : " .. tab.data.result, math.random(0, 200), "翻译")
                                end
                            end
                        end
                    else
...
----谷歌线路
menu.toggle(google_list, "开启", {},"翻译器将监听传入的消息并进行翻译", function(on)
    config.translateOn = on
end)
menu.toggle(google_list, "翻译自己", {}, "翻译您自己发送的消息", function(on)
    config.translateSelf = on
end)

local blacklistMenu = menu.list(google_list, "翻译语言黑名单", {},"在列表中开启的语言的消息将被忽略")
...
menu.list_select(google_list, "目标语言", {}, "",LangNameList, 1, function(index)
    config.targetLanguageIncoming = LangKeyList[index]
end)

menu.toggle_loop(google_list, "发送翻译文本", {}, "仅使用T or Y即可键入文本\n当使用此选项时你应该首选关闭翻译自我", function()
    if PAD.IS_CONTROL_PRESSED(0,245) or PAD.IS_CONTROL_PRESSED(0,246) then
        local label = util.register_label("输入文本")
        local input = get_input_from_screen_keyboard(label, 254, "")
        if input == "" then return end
...
menu.divider(chat_transl, "设置")
    local common_settings = menu.list(chat_transl, "通用设置", {},"")
    local TranslatedMsgLocationOptions = {{1,"团队聊天未联网"},{2,"团队聊天已联网"},{3,"全球聊天未联网"},{4,"全球聊天已联网"},{5,"通知"}}
    ...
    local trans_col_name = {{1,"纯白"},{2,"白色"},{3,"黑色"},{4,"灰色"},{5,"浅灰色"},{6,"暗灰色"},{7,"红色"},{8,"浅红色"},{9,"暗红色"},{10,"蓝色"},{11,"浅蓝色"},{12,"暗蓝色"},{13,"黄色"},{14,"浅黄色"},{15,"暗黄色"},{16,"橙色"},{17,"浅橙色"},{18,"暗橙色"},{19,"绿色"},{20,"浅绿色"},{21,"暗绿色"}}
    menu.list_select(common_settings, '翻译文本颜色', {''}, '', trans_col_name, 16, function (c)
        transl_text_col = c - 1
    end)
