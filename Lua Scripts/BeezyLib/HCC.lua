...
--- Lua Script Settings
    ...
    HC_DIR = filesystem.scripts_dir() .. "/HeezyScript/" .. '/Heist Control/'
        
        FolderDirs = {
            Img = HC_DIR .. "Image\\",
            Setting = HC_DIR .. "Setting\\",
            Lang = HC_DIR .. "Language\\",
            HaxUI = HC_DIR .. "HeezyUI\\",
        }
...
--- Main Lists
        ...
        local makemoneyfast = menu.list(HeistControl_features, "快速任务", {}, "此选项可帮助您快速完成各种高收益的抢劫任务")
        ...
        PERICO_HEIST = menu.list(HeistControl_features, TRANSLATE("Cayo Perico Heist"), {"hccp"}, "#结算任务上限为255万\n#若超过上限会不到账喔(队友同理)\n#以下内容若麻烦推荐使用[自动预设]\n\n公式: (抢劫金额) 乘 (分红百分比) 减 (帕维尔的30万) = 255万\n举例:如果您选择50%的分红收入,您的抢劫金额最大收益上限应为560万左右,建议使用高级功能修改背包容量\n\n[丢丢小课堂]", function();  end)
        CASINO_HEIST = menu.list(HeistControl_features, TRANSLATE("Diamond Casino Heist"), {"hccah"}, "#结算任务上限为360万\n#若超过上限会不到账喔(队友同理)\n#以下内容若麻烦推荐使用[自动预设]\n#注意:困难模式下,如果您选择100%的分红,最终会得到290万左右的金额\n#切记:请自行叠加分红百分比,若超过上限360万将得到0收入!!!\n\n[丢丢小课堂]", function(); end)
        DOOMS_HEIST = menu.list(HeistControl_features, TRANSLATE("Doomsday Heist"), {"hcdooms"}, "#结算任务上限为255万\n#若超过上限会不到账喔(队友同理)\n#以下内容若麻烦推荐使用[自动预设]\n\n#注意:如果您选择100%的分红\n末日一到账金额: 120万左右\n末日二到账金额: 180万左右\n末日三到账金额: 230万左右\n[以上皆为困难模式]\n#切记:请自行叠加分红百分比,若超过上限255万将得到0收入!!!\n\n[丢丢小课堂]", function(); end)
        CLASSIC_HEISTS = menu.list(HeistControl_features, TRANSLATE("Classic Heist"), {"hcclassic"}, "#全福银行抢劫结算金额为1500万\n#您必须是房主并且仅允许您个人得到1500万!!!", function(); end)
        ...
        MASTER_UNLOCKR = menu.list(HeistControl_features, TRANSLATE("Master Unlocker"), {"hcmu"}, "解锁游戏内容/DLCs/彩蛋/奖品等..", function(); end)
        TOOLS = menu.list(HeistControl_features, TRANSLATE("Tools"), {"hctool"}, "#实用选项(任务)\n包含自动赚钱/快速完成/管理选项等..", function(); end)
		...
		menu.action(autocayosolo, TRANSLATE("一键完成任务"), {"hccpfintar"}, IS_WORKING(true) .. "(" .. TRANSLATE("Cayo Perico Heist") .. " > " .. TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Compound") .. " > " .. TRANSLATE("Primary Target") .. ")", function() -- https://www.unknowncheats.me/forum/3418914-post13398.html
            SET_INT_LOCAL("fm_mission_controller_2020", 45450 + 1, 51338752) -- CutterStage
            SET_INT_LOCAL("fm_mission_controller_2020", 45450 + 1378 + 1, 50) -- BitCheck
        end)
		...
		menu.action(autocayomult, TRANSLATE("一键完成任务"), {"hccpfintar"}, IS_WORKING(true) .. "(" .. TRANSLATE("Cayo Perico Heist") .. " > " .. TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Compound") .. " > " .. TRANSLATE("Primary Target") .. ")", function() -- https://www.unknowncheats.me/forum/3418914-post13398.html
            SET_INT_LOCAL("fm_mission_controller_2020", 45450 + 1, 51338752) -- CutterStage
            SET_INT_LOCAL("fm_mission_controller_2020", 45450 + 1378 + 1, 50) -- BitCheck
        end)
...
--- Tools
    ...
    STAT_EDITOR_READER = menu.list(TOOLS, TRANSLATE("Stat Editor And Reader"), {}, "", function(); end)
        ...
        GTAHAXUI_STAT_EDITOR = menu.list(STAT_EDITOR_READER, TRANSLATE("HeezyUI Stat Editor"), {}, TRANSLATE("HeezyUI is a free mod menu for editing stats, globals, and locals from UnknownCheats. Heist Control can help you using customizable HeezyUI's stat txt files. Note that you can download the files by googling easily."), function(); end)
                ...
                menu.divider(GTAHAXUI_STAT_EDITOR_LIST, TRANSLATE("Load Custom Stat Files"))
                    ...
                    for idx, file_dir in pairs(filesystem.list_files(FolderDirs.HaxUI)) do
                        if string.contains(file_dir, ".txt") then 
                            menu.list(GTAHAXUI_STAT_EDITOR_LIST, DIR_TO_FILE_NAME("HeezyUI", file_dir), { "hchaxui" .. idx }, "", function(); end)
                            table.insert(HaxUIFiles, DIR_TO_FILE_NAME("HeezyUI", file_dir))
                        end
                    end
                    ...
                    for idx1, file_name in pairs(HaxUIFiles) do
                        ...
                        for _, file_content in pairs(Contents) do
                            for __, stat_type in pairs(StatTypes) do
                                if string.contains(file_content, stat_type) then -- If Heist Control detects HeezyUI's file stat type
                        ...
                        if HelpTexts.Type == "N/A" then
                            HelpTexts.Comment = TRANSLATE("Please check this file has been typed for HeezyUI. Applying it won't affect anything in GTAV.")
                        end
            ...
            menu.hyperlink(GTAHAXUI_STAT_EDITOR, TRANSLATE("Visit HeezyUI"), "https://www.unknowncheats.me/forum/grand-theft-auto-v/461672-gtahax-1-58-external-thread-3-a.html", "")
