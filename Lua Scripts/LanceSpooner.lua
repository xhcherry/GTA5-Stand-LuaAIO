...
if not filesystem.is_dir(store_dir) then
    ...
    if not SCRIPT_SILENT_START then
        util.toast("欢迎来到 LanceSpooner!")
        util.toast("将您的载具和地图模组放入Lua Scripts / store / LanceSpooner / vehicles or maps :)")
    end
end
...
if filesystem.is_dir(filesystem.scripts_dir() .. '\\menyoo vehicles') or filesystem.is_dir(filesystem.scripts_dir() .. '\\menyoo maps') and not SCRIPT_SILENT_START then
    util.toast("发现你有版本很老的menyoo载具目录!.")
    util.toast("将您的载具和地图模组放入 Lua Scripts / store / LanceSpooner / vehicles or maps 才能生成 :). 删除旧目录以删除此消息.")
end
...
menu.divider(menu.my_root(), "载具")

menu.action(menu.my_root(), "搜索载具模组", {"lssearchv"}, "", function(click_type)
...
local load_vehicle_action = menu.list_action(menu.my_root(), "加载模组载具", {}, "加载载具. " .. instruction_text, all_vehicles, function(index, value)
...
loaded_vehicles_root = menu.list(menu.my_root(), "已加载的载具", {"lancespoonerloadedvehicles"}, "你已经加载的载具")


menu.divider(menu.my_root(), "地图")
menu.action(menu.my_root(), "搜索地图模组", {"lssearchmap"}, "", function(click_type)
...
local load_map_action = menu.list_action(menu.my_root(), "加载XML地图", {}, "加载地图. " .. instruction_text, all_maps, function(index, value)
...
loaded_maps_root = menu.list(menu.my_root(), "已加载的地图", {"lancespoonerloadedmaps"}, "你已经加载的地图")


menu.divider(menu.my_root(), "收藏夹")

local load_favorite_vehicle_action = menu.list_action(menu.my_root(), "最爱的载具", {}, "加载载具.", favorite_vehicles, function(index, value)
    load_vehicle(vehicles_dir .. '\\' .. value)
end)

local load_favorite_map_action = menu.list_action(menu.my_root(), "最爱的地图", {}, "加载地图.", favorite_maps, function(index, value)
...
v_search_results = {}
v_search_results_action = menu.list_action(menu.my_root(), "载具搜索结果", {"lsvsearchresults"}, "", v_search_results, function(index, value)
    local path = vehicles_dir .. '\\' .. value
    load_vehicle(path)
end)

m_search_results = {}
m_search_results_action = menu.list_action(menu.my_root(), "地图搜索结果", {"lsmsearchresults"}, "", m_search_results, function(index, value)
    local path = maps_dir .. '\\' .. value
    menyoo_load_map(path)
end)

async_http.init("pastebin.com", "/raw/nrMdhHwE", function(result)
    menu.hyperlink(menu.my_root(), "加入官网discord", result, "")
end)
async_http.dispatch()
