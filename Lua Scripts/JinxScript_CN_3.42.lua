...
local function player(pid) 
    ...
    local jesus_ped
    menu.toggle(trolling, "悲伤的耶稣", {""}, "召唤耶稣攻击TA.", function(toggled)
        if toggled then
            local ped = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pid)
    ...
    menu.action(trolling, "发射玩家", {}, "警告:这可能会导致崩溃出现,但概率极低.主要是由于垃圾邮件,所以请不要乱扔垃圾.", function()																																																	   
...
jesus_toggle = menu.toggle(jesus_main, "启用", {}, "", function(toggle)
    ...
    if toggled then
        ...
        if HUD.IS_WAYPOINT_ACTIVE() then
            ...
            TASK.TASK_VEHICLE_DRIVE_TO_COORD_LONGRANGE(jesus_ped, vehicle, pos, 20.0, style, 0.0)
        else
