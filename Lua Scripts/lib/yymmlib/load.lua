...
util.keep_running()
util.show_corner_help("~g~欢迎使用夜幕Lua 此版本号为：~b~3.9~g~‹\n ~o~希望夜幕lua带给你快乐！")
...
function xianshijiaoben1(state)
    ...
    draw_string(string.format("~italic~¦~bold~欢迎使用夜幕-V3.9"), 0.35,0.08, 0.6,5)
    util.yield()
    end
end
local colors = {
...
notification("你好，尊贵的夜幕Lua用户", colors.green)
util.on_stop(function()
	notification("下次遇见会更好", colors.blue)
end)
local festive_div = menu.divider(menu.my_root(), "运行夜幕Lua", {}, "")
