menu.action(zhuti, "加载推荐的设置", {}, "如果这是你第一次使用, 您需要运行此程序以获得最佳体验.", function()
    menu.trigger_commands("blur 0;borderwidth 4;borderrounded off;tabswidth 56;tabsposition top;tabsalignment centre;tabslefticon off;tabsname off;tabsrighticon off;menuwidth 392;primary 00000000;commandinfotextposition bottom;spacer 4;border 191919;background 191919")
    util.toast("完成! 我建议现在就拯救你的国家 (i.e \"savemain\" in the command box).")
end)
local clr = {r=1,g=0.2,b=0.6,a=1}
menu.rainbow(
    menu.colour(zhuti, "原色", {"primarycol"}, "", clr, true, function(val)
        clr = val
    end)
)
...
local timer = time()
local pulsel = true
menu.toggle(zhuti,"选择改变时重置动画", {}, "", function(st) pulsel = st end, pulsel)
local animsp = 0.001
menu.slider(zhuti,"动画速度", {"animationspeed"}, "", -2147483648, 2147483647, 100, 20, function(val) timer = time() animsp = val/100000 end)
local sep_height = 2/1080
menu.slider(zhuti, "分离器高度", {"seperatorheight"}, "", 0, 16, 2, 1, function(val) sep_height = val/1080 end)
local last_slot
...
util.create_tick_handler(function()
    ...
    local bg_clr = {
        r=menu.get_value(bg_ref_r)/255,
        g=menu.get_value(bg_ref_g)/255,
        b=menu.get_value(bg_ref_b)/255,
