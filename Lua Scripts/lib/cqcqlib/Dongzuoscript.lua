...
function download_resources_update(filepath, destOverwritePath)
    ...
    if filesystem.exists(lockPath) then
        util.log(SCRIPT_NAME .. ": 跳过资源更新 \" .. lib .. \", 发现更新文件")
    end
    local lock = io.open(lockPath, "w")
    if lock == nil then
        util.toast(SCRIPT_NAME .. ": 不能创建文件，跳过更新", TOAST_ALL)
        return
    end
    lock:close()
    async_http.init("jackz.me", "/stand/get-lua.php?script=resources/" .. filepath .. "&source=" .. SCRIPT_SOURCE .. "&branch=" .. (SCRIPT_BRANCH or "master"), function(result, res_headers, status_code)
        ...
        if file == nil then
            util.toast("无法写入资源文件: " .. filepath .. "\n详情请见日志")
            util.log(string.format("%s: Resource \"%s\" 无法创建文件.", SCRIPT_NAME, filepath))
            return
        end
        Log.log("更新资源 ", filepath, "for", SCRIPT_NAME, "to branch", SCRIPT_BRANCH or "master")
        file:write(result:gsub("\r", "") .. "\n")
        file:close()
        util.toast(SCRIPT .. ": 自动更新资源 '" .. filepath .. "'")
    end, function(e)
        os.remove(lockPath)
        util.toast(SCRIPT .. " 无法加载: 库文件缺失. (" .. filepath .. ")", 10)
        util.stop_script()
    end)
    async_http.dispatch()
end
...
function get_version_info(version)
    if not version then error("缺失版本", 2) end
...
function compare_version(a, b)
    ...
    if not av or not bv then error("比较缺失版本") end
        ...
        if versions[SCRIPT_NAME] == nil or compare_version(VERSION, versions[SCRIPT_NAME]) == 1 then
            if versions[SCRIPT_NAME] ~= nil then
                async_http.init("jackz.me", "/stand/changelog.php?raw=1&script=" .. SCRIPT_NAME .. "&since=" .. versions[SCRIPT_NAME] .. "&branch=" .. (SCRIPT_BRANCH or "master"), function(result)
                    util.toast("更新日志 " .. SCRIPT_NAME .. " version " .. VERSION .. ":\n" .. result)
                end, function() util.log(SCRIPT_NAME ..": 无法获得更新日志") end)
                async_http.dispatch()
            end
            versions[SCRIPT_NAME] = VERSION
            versionFile:seek("set", 0)
            versionFile:write("# 请勿编辑! 文件是用来做更新日志的\n")
...
SCRIPT_META_LIST = menu.list(dzjb, "脚本信息")
menu.divider(SCRIPT_META_LIST, SCRIPT_NAME .. " 是最" .. VERSION)
menu.hyperlink(SCRIPT_META_LIST, "加入脚本中文交流", "https://jq.qq.com/?_wv=1027&k=YneNsXUT")
menu.action(SCRIPT_META_LIST, "欢迎加入", {}, "欢迎加入中文交流群！", function()
    util.toast("快进来！别不知好歹！！")
end)
--menu.hyperlink(SCRIPT_META_LIST, "查看完整的更新日志", "https://jackz.me/stand/changelog?html=1&reverse=1&script=" .. SCRIPT_NAME)
--menu.hyperlink(SCRIPT_META_LIST, "Jackz's 公会", "https://www.guilded.gg/i/k8bMDR7E?cid=918b2f61-989c-41c4-ba35-8fd0e289c35d&intent=chat", "获得帮助，提交建议，报告错误，或者与其他用户进行击剑")
--menu.hyperlink(SCRIPT_META_LIST, "Github 资源", "https://github.com/Jackzmc/lua-scripts", "View all my lua scripts on github")
--if SCRIPT_SOURCE == "MANUAL" then
   -- menu.list_select(SCRIPT_META_LIST, "更新频道", {SCRIPT_NAME.."channel"}, "此脚本的更新频道.\n更改频道可能会导致错误.", SCRIPT_BRANCH_NAMES, 1, function(index, name)
     --   show_busyspinner("下载更新...")
      --  download_script_update(SCRIPT_BRANCH_IDS[index], function()
       --     HUD.BUSYSPINNER_OFF()
          --  util.log(SCRIPT_NAME .. ": 更新频道更改为 " .. SCRIPT_BRANCH_IDS[index])
        --    util.toast("更新频道更改为 " .. name .. " (" .. SCRIPT_BRANCH_IDS[index] .. ")\n重新加载以应用更新")
      --  end, function()
        --    util.toast("无法下载更新频道的最新版本.")
      --  end)
   ---- end)
--else
  --  menu.readonly(SCRIPT_META_LIST, "更新频道", "粘贴手动版 https://jackz.me/stand/get-latest-zip 到页面.")
--end
--if _lang ~= nil then
  --  menu.hyperlink(SCRIPT_META_LIST, "协助翻译", "https://jackz.me/stand/translate/?script=" .. SCRIPT, "如果你愿意帮忙翻译,这个脚本的默认翻译是通过谷歌翻译提供的,但你可以在这里编辑:\n一旦你做了修改,右上角有一个保存键用于获取 -CHANGES.json 文件, 发送给我.")
   -- _lang.add_language_selector_to_menu(SCRIPT_META_LIST)
--end
--menu.readonly(SCRIPT_META_LIST, "建议提交", BRANCH_LAST_COMMIT and BRANCH_LAST_COMMIT:sub(1,10) or "开发建议")
--menu.action(SCRIPT_META_LIST, "上传日志", {}, "上传你的日志最后约20行 (%appdata%\\Stand\\Log.txt) 到 hastebin.com.\nHastebin会在7天后过期.\n\n上传的日志可以从 \"Open Uploaded Log\" 按一下键", function()
   -- local logs = io.open(filesystem.stand_dir() .. "Log.txt", "r")
   -- if logs then
      --  show_busyspinner("上传日志....")
     --   async_http.init("hastebin.com", "/documents", function(body)
          --  HUD.BUSYSPINNER_OFF()
        --    local url = "https://hastebin.com/" .. body:sub(9, -3)
         --   util.toast("Uploaded: " .. url)
         --   menu.hyperlink(SCRIPT_META_LIST, "打开上传日志", url)
       -- end, function()
         --   util.toast("提交日志失败，网络错误")
        --    HUD.BUSYSPINNER_OFF() 
     --   end)
       -- logs:seek("end", -2048)
...
local status = pcall(require, 'resources/cqcqscript/cqcqaction')
if not status then
    util.toast("加载资源文件失败: cqcqscript/cqcqaction.lua")
    util.stop_script()
end
--if ANIMATION_DATA_VERSION ~= SPECIAL_ANIMATIONS_DATA_FILE_VERSION then
    --if SCRIPT_SOURCE == "MANUAL" then
      --  download_resources_update("jackz_actions/actions_data.min.lua", "jackz_actions/actions_data.lua")
       -- util.toast("重启脚本以使用更新的资源文件")
   -- else
      --  util.toast("jackz_actions: 警告: 过时或缺失 actions_data. 版本: " .. (ANIMATION_DATA_VERSION or "<missing>"))
      --  util.stop_script()
  --  end
--end
...
menu.action(dzjb, "停止所有动作", {"stopself"}, "停止当前场景或动画", function(v)
    clear_anim_props()
...
menu.toggle(dzjb, "立即清除", {"clearimmediately"}, "如果启用，将在激活新动画/场景时立即停止正在播放的动画/场景。 如果为关，您将顺利过渡到下一个动作.", function(on)
    lclearActionImmediately = on
end, clearActionImmediately)
menu.list_select(dzjb, "行动目标", {"actiontarget"}, "将执行此操作的实体.\n0 = 只有你自己\n1 = 只有NPC\n2 = 你和NPC", { { "自己" }, { "NPC" }, {"全部"} }, 1, function(index)
    affectType = index - 1
end)
...
menu.divider(dzjb, "动作选项")
local specialAnimationsMenu = menu.list(dzjb, "特殊动画", {}, "可以使用道具的特殊动画")
menu.toggle(specialAnimationsMenu, "可控", {"animationcontrollable"}, "动画是否允许玩家控制?", onControllablePress, allowControl)
local animationsMenu = menu.list(dzjb, "动画", {}, "您可以播放的动画列表")
...
local scenariosMenu = menu.list(dzjb, "场景", {}, "您可以加载的场景列表\n某些场景仅适用于某些性别，例如 AA Coffee 仅适用于男性人物.")
...
local ambientSpeechMenu = menu.list(dzjb, "环境语音", {}, "允许您对自己或其他NPC进行环境语音")
...
-----------------------
util.toast("在动画上按住 LEFT SHIFT 可将其从收藏夹中添加或删除.", 2)
util.toast(string.format("人物动作脚本 %s by Jackz.", VERSION), 2)
