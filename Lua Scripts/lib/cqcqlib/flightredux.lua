...
local store_dir = filesystem.scripts_dir() .. '\\HeezyScript\\' .. '\\flightredux\\'
...
hhud_dir = filesystem.scripts_dir() .. '\\HeezyScript\\' .. '\\flightredux\\'
...
gpws = true
menu.toggle(planehud, "警报", {"gpws"}, "", function(on)
    gpws = on
end, true)
...
menu.toggle(planehud , "飞机HUD显示", {"aircrafthuddisplay"}, "", function(state)
    UItoggle = state
while UItoggle do 
    mst_warn = ""
    ...
    if player_vehicle ~= 0 then
        ...
        if PED.IS_PED_IN_ANY_PLANE(players.user_ped()) or PED.IS_PED_IN_ANY_HELI(players.user_ped()) then
            ...
            directx.draw_text(0.71, 0.42, "高度 " .. tostring(math.ceil(altitude)), 4, 1.2, hud_green, false)
            -- gear 
            if VEHICLE.GET_LANDING_GEAR_STATE(player_vehicle) == 0 then
                directx.draw_text(0.50, 0.40, "挡位", 5, 0.7, hud_green, false)
            end
            -- airspeed
            local knts = (ENTITY.GET_ENTITY_SPEED(player_vehicle) * 2.236936)*0.868976
            -- directional
            local ang = ENTITY.GET_ENTITY_ROTATION(player_vehicle, 0)
            directx.draw_text(0.71, 0.46, "地平线 " .. tostring(math.ceil(ang.x)), 4, 0.7, hud_green, false)
            directx.draw_text(0.71, 0.48, "水平线 " .. tostring(math.ceil(ang.Y)), 4, 0.7, hud_green, false)
            directx.draw_text(0.50, 0.80, "机体航向 " .. tostring(math.ceil(ang.y)), 5, 0.7, hud_green, false)
            -- mach
            directx.draw_text(0.250, 0.62, "马赫 " .. tostring(math.ceil(knts * 0.00149984)), 4, 0.8, hud_green, false)
            directx.draw_texture(altimeter_outline, 0.020, 0.020, 0.0, 0.0, 0.3, 0.44, 0.5, hud_green)
            directx.draw_text(0.195, 0.42, tostring(math.ceil(knts)) .. " KM/H", 4, 1.2, hud_green, false)
            ...
            if mode == "火箭" then 
                ...
                if lockon_state == 1 then 
                    ...
                    directx.draw_text(0.61, 0.71, "锁定", 4, 0.8, hud_green, false)
                elseif lockon_state == 2 then
                    if blink_this_frame then
                        directx.draw_text(0.61, 0.71, "锁定了", 4, 0.8, hud_red, false)
                    end
            ...
            pluto_switch mst_warn do
                case "FIRE": 
                    if blink_this_frame then
                        directx.draw_text(0.5, 0.9, "失火", 5, 3, hud_red, false)
                    end
                    ...
                    break
                case "MASTER WARN":
                    directx.draw_text(0.5, 0.9, "主要警告", 5, 3, hud_red, false)
                    if play_sound_this_frame then 
                ...
                case "BOGEY LOCK-ON":
                    if f_blink_this_frame then 
                        directx.draw_text(0.5, 0.9, "敌机锁定", 5, 3, hud_red, false)
                    end
                    break
                case "TERRAIN":
                    if f_blink_this_frame then 
                        directx.draw_text(0.5, 0.9, "高度过低", 5, 3, hud_red, false)
                    ...
                    break
                case "SINK RATE":
                    directx.draw_text(0.5, 0.9, "正在下降", 5, 3, hud_yellow, false)
                    if play_sound_this_frame then 
                        AUDIO.PLAY_SOUND_FRONTEND(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", true)
                    end
                    break
                case "MISSILE": 
                    directx.draw_text(0.5, 0.9, "导弹预警", 5, 3, hud_red, false)
                    --[[if play_sound_this_frame then 
