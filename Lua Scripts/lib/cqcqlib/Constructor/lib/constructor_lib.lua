require "lib.cqcqlib.natives-1663599433"
...
local status_inspect, inspect = pcall(require, "lib.cqcqlib.constructor.lib.inspect")
if not status_inspect then error("Could not load inspect lib. This should have been auto-installed.") end

local status_constants, constants = pcall(require, "lib.cqcqlib.constructor.lib.constants")
...
constructor_lib.serialize_entity_attributes = function(attachment)
    ...
    attachment.options.object_tint = ABC_OBJECT.GET_OBJECT_TINT_INDEX(attachment.handle)
end
...
constructor_lib.deserialize_entity_attributes = function(attachment)
    ...
    if attachment.num_bones == nil or attachment.num_bones == 200 then attachment.num_bones = ABC_ENTITY.GET_ENTITY_BONE_COUNT(attachment.handle) end
...
constructor_lib.make_entity_networked = function(attachment)
    ...
    ABC_ENTITY.SET_ENTITY_SHOULD_FREEZE_WAITING_ON_COLLISION(attachment.handle, false)
