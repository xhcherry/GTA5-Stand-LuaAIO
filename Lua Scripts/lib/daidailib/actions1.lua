...
menu.action(action_list_lua, "停止所有动作", {"stopself"}, "停止当前场景或动画", function(v)
    clear_anim_props()
...
menu.toggle(action_list_lua, "立即清除", {"clearimmediately"}, "如果启用，将在激活新动画/场景时立即停止正在播放的动画/场景。 如果为关，您将顺利过渡到下一个动作.", function(on)
    lclearActionImmediately = on
end, clearActionImmediately)
menu.list_select(action_list_lua, "行动目标", {"actiontarget"}, "将执行此操作的实体.\n0 = 只有你自己\n1 = 只有NPC\n2 = 你和NPC", { { "自己" }, { "NPC" }, {"全部"} }, 1, function(index)
    affectType = index - 1
end)
