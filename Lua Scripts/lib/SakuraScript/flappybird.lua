...
local tube_file = "SakuraScript\\FlappyStand\\tube.png" -- default file name
...
menu.divider(flappygame, "管道设置")
 menu.slider(flappygame, "更改管道风格", {"tubestyle"}, "", 1, 3, 1, 1, function(style)
	if style == 1 then
	tube_file = "SakuraScript\\FlappyStand\\tube.png"
	tube_tex = directx.create_texture(filesystem.resources_dir() .. tube_file) 
	end
	if style == 2 then
	tube_file = "SakuraScript\\FlappyStand\\tube_g.png"
	tube_tex = directx.create_texture(filesystem.resources_dir() .. tube_file) 
	end
	if style == 3 then
	tube_file = "SakuraScript\\FlappyStand\\tube_pain.png"
	tube_tex = directx.create_texture(filesystem.resources_dir() .. tube_file) 
	end
end)
