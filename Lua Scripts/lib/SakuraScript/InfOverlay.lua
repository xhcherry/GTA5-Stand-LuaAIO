...
--menu
players_info = menu.toggle(play_info,"开启",{},"[配置]",function(toggled)
    infoverplaytoggle(toggled)
end);set_menu_value(players_info, luaConfig.players_info, false)

local renderwindow = false
infoverlay = menu.list(play_info, "设置", {}, '', function()
    renderwindow = true 
end, function()
    renderwindow = false
end)
...
--默认位置
local guix = (luaConfig.infoverlay_x or 0) /1000
local guiy = (luaConfig.infoverlay_y or 0) /1000
infoverlay_x = menu.slider(infoverlay, 'X:', {'overlayx'}, '[配置]\n信息覆盖的水平位置.', 0, 1000, (luaConfig.infoverlay_x or 0), 1, function(s)
    guix = s/1000
end)
infoverlay_y = menu.slider(infoverlay, 'Y:', {'overlayy'}, '[配置]\n信息覆盖的垂直位置.', 0, 1000, (luaConfig.infoverlay_y or 0), 1, function(s)
    guiy = s/1000
end)
...
----main function
function infoverplaytoggle(toggled)
    player_info = toggled
    if player_info then
        create_blurrect()
    end
    while player_info do
