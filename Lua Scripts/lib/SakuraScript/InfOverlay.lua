...
menu.set_value(players_info, luaConfig.players_info) --配置
...
--默认位置
local guix = luaConfig.infoverlay_x /1000
local guiy = luaConfig.infoverlay_y /1000
infoverlay_x = menu.slider(infoverlay, 'X:', {'overlayx'}, '配置[√]\n信息覆盖的水平位置.', 0, 1000, luaConfig.infoverlay_x, 1, function(s)
    guix = s/1000
end)
infoverlay_y = menu.slider(infoverlay, 'Y:', {'overlayy'}, '配置[√]\n信息覆盖的垂直位置.', 0, 1000, luaConfig.infoverlay_y, 1, function(s)
    guiy = s/1000
end)
