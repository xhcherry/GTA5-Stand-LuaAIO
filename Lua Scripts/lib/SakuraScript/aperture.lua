...
local function cam_control()
    ...
    --设置小地图实时位置
    local campos = ENTITY.GET_ENTITY_COORDS(cam_obj, false)
    ...
    local mod_y = 0
    local mod_z = 0
    mod_x = PAD.GET_DISABLED_CONTROL_NORMAL(2, 2) * 5
    mod_z = -PAD.GET_DISABLED_CONTROL_NORMAL(1, 1) * 5
    local cam_rot = ENTITY.GET_ENTITY_ROTATION(cam_obj, 0)

    if PAD.IS_CONTROL_PRESSED(52, 52) then 
        if cam_rot.y > -45 then
            mod_y = mod_y - move_speed_cam
        end
    elseif PAD.IS_CONTROL_PRESSED(54, 54) then 
        if cam_rot.y < 45 then
            mod_y = mod_y + move_speed_cam
        end
    elseif PAD.IS_CONTROL_PRESSED(250, 250) then 
        cam_rot.y = 0
        cam_rot.x = 0
    end

    ENTITY.SET_ENTITY_ROTATION(cam_obj, cam_rot.x - mod_x, cam_rot.y + mod_y, cam_rot.z + mod_z)
...
local function cam_toggled(on)
    ...
    if on then
        ...
        HUD.UNLOCK_MINIMAP_POSITION() --关闭小地图实时位置
...
menu.toggle(free_cam, "相机开启", {}, "wasd 用于平移摄像机,shift/ctrl 用于上下移动,Q/E 用于旋转,R 用于重置,space 用于快速移动,alt 用于精确移动", function(on)
    cam_toggled(on)
end)
