...
local sa_fov = 2
local show_box = true --显示方框
local show_line = false --显示射线
local show_pointer = false ---显示指针
...
menu.toggle_loop(silent_aimbotroot, "开启自瞄", {}, "", function(toggle)
    ...
    if target ~= 0 and PLAYER.IS_PLAYER_FREE_AIMING(players.user()) then
        ...
        if show_box then
            draw_bounding_box(target, false, {r = 255, g = 255, b = 255, a = 255})
        end
...
set_view = menu.list(silent_aimbotroot, "设置视野", {}, "")
    ...
    menu.slider(set_view, "视野大小", {}, "推荐视野2", 0, 270, 2, 1, function(s)
        sa_fov = s
    end)
...
menu.toggle(silent_aimbotroot, "显示方框", {}, "标记自瞄对象", function(on)
    show_box = on
end, true)
