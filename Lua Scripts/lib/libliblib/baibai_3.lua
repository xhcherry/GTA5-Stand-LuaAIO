...
local NEAR_PED_CAM = menu.list(entity_options, "管理附近的NPC和摄像头", {}, "")
...
menu.textslider_stateful(MISSION_ENTITY_custom, "实体类型", {}, "点击应用修改", { "Ped", "Vehicle", "Object", "Pickup" }, function(value)
    if value == 1 then
...
peds_root = menu.list(entity_options, "控制行人", {"baibaipeds"}, "")
...
rain_peds = false
menu.toggle(peds_root, "行人雨", {"rainpeds"}, "...", function(on)
rain_peds = on
...
php_bars = false
menu.toggle(peds_root, "HP条", {"pedhpbars"}, "在NPC上面画血量条。", function(on)
php_bars = on
mod_uses("ped", if on then 1 else -1)
