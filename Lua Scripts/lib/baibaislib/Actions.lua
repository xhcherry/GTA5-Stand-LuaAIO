...
VERSION = "1.10.10"
local ANIMATIONS_DATA_FILE = filesystem.scripts_dir().. "/baibaiScript/actions/animations.txt"
...
local status = pcall(require, 'lib.baibailib.playerlib')
if not status then
    util.toast("加载资源文件失败: lib.baibailib.playerlib")
    util.stop_script()
end
...
function create_self_speech_ped()
    ...
    ENTITY.ATTACH_ENTITY_BONE_TO_ENTITY_BONE(ped, my_ped, 0, 0, 0, 0)
...
function download_animation_data()
    ...
    async_http.init("jackz.me", "/stand/baibaiScript/actions/animations.txt", function(result)
