...
local function get_player_vehicle_in_control(pid, opts)
    ...
    local dist = POLICECAR_SYSTEM.VDIST2(pos1.x, pos1.y, 0, pos2.x, pos2.y, 0)
...
local function save_headlights(vehicle)
    saveData.Headlights_Color = VEHICLE.GET_VEHICLE_XENON_LIGHT_COLOR_INDEX(vehicle)
...
local function restore_headlights(vehicle)
    VEHICLE.SET_VEHICLE_XENON_LIGHT_COLOR_INDEX(vehicle, saveData.Headlights_Color)
...
local function save_neon(vehicle)
    ...
    VEHICLE.GET_VEHICLE_NEON_COLOUR(vehicle, Color.r, Color.g, Color.b)
    saveData.Lights.Neon = {
        ...
        Left = VEHICLE.GET_VEHICLE_NEON_ENABLED(vehicle, 0),
        Right = VEHICLE.GET_VEHICLE_NEON_ENABLED(vehicle, 1),
        Front = VEHICLE.GET_VEHICLE_NEON_ENABLED(vehicle, 2),
        Back = VEHICLE.GET_VEHICLE_NEON_ENABLED(vehicle, 3),
    }
end
...
local function restore_neon(vehicle)
    VEHICLE.SET_VEHICLE_NEON_ENABLED(vehicle, 0, saveData.Lights.Neon.Left or false)
    VEHICLE.SET_VEHICLE_NEON_ENABLED(vehicle, 1, saveData.Lights.Neon.Right or false)
    VEHICLE.SET_VEHICLE_NEON_ENABLED(vehicle, 2, saveData.Lights.Neon.Front or false)
    VEHICLE.SET_VEHICLE_NEON_ENABLED(vehicle, 3, saveData.Lights.Neon.Back or false)
    VEHICLE.SET_VEHICLE_NEON_COLOUR(vehicle, saveData.Lights.Neon.Color.r, saveData.Lights.Neon.Color.g, saveData.Lights.Neon.Color.b)
end
...
local function activate_lights(vehicle)
    ...
    AUDIO.TRIGGER_SIREN_AUDIO(vehicle, true)
    AUDIO.SET_SIREN_BYPASS_MP_DRIVER_CHECK(vehicle, true)
    for _, attachment in pairs(attachments) do
...
local function deactivate_lights(vehicle)
    ...
    AUDIO.SET_SIREN_BYPASS_MP_DRIVER_CHECK(vehicle, false)
    VEHICLE.SET_VEHICLE_SIREN(vehicle, false)
    for _, attachment in pairs(attachments) do
...
local function policify_tick_ying()
    if is_active_lights then
        if override_headlights then
            VEHICLE.SET_VEHICLE_XENON_LIGHT_COLOR_INDEX(policified_vehicle, 8)
        end
        if override_neon then
            VEHICLE.SET_VEHICLE_NEON_COLOUR(policified_vehicle, 0, 0, 255)
        end
    end
    for _, attachment in pairs(attachments) do
...
local function policify_tick_yang()
    if is_active_lights then
        if override_headlights then
            VEHICLE.SET_VEHICLE_XENON_LIGHT_COLOR_INDEX(policified_vehicle, 1)
        end
        if override_neon then
            VEHICLE.SET_VEHICLE_NEON_COLOUR(policified_vehicle, 255, 0, 0)
        end
    end
    for _, attachment in pairs(attachments) do
        if attachment.flash_start_on ~= nil then
...
--- Menu Options
---
menu.toggle(policecar, "警用车辆", {"policify"}, "把当前载具设置成警用车辆", function(on)
    if on then
...
menu.action(policecar, "警笛警告", {"blip"}, "快速的警笛声", function()
    for _, attachment in pairs(attachments) do
...
--menu.action(policecar, "Police Report", {}, "Play police report", function()
...
menu.list_select(policecar, "警笛与灯光", {}, "", {"关", "灯光", "警笛与灯光"}, 1, function(key)
    if key == 1 then
...
menu.action(policecar, "呼叫支援", {}, "呼叫附近单位支援", function(toggle)
...
local attach_additional_lights_menu = menu.list(policecar, "附加附件")
edit_attachments_menu = menu.list(policecar, "编辑附件")
...
local options_menu = menu.list(policecar, "选项")
