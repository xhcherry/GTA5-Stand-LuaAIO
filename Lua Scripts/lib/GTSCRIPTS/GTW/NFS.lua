-- NFS speedometer by CocoW#6969 v0.3
...
menu.toggle(nfs,"Change speedometer look", {"splook"}, "Change speedometer look to either normal or need for speedometer",function(pog)	
	if pog then
...
menu.rainbow(menu.colour(nfs, "Change speedometer accent colour", {"spc"}, "", sp_colour, true, function(colour)
	sp_colour = colour
end))
menu.colour(nfs, "Change speedometer text colour", {"sptxtc"}, "", txt_colour, true, function(colour)
	txt_colour = colour
end)
menu.slider(nfs, "Speedometer pos X", {"spX"}, "", 1, 1000, 930, 1, function(x)
	sp_posX=x/1000
end)
menu.slider(nfs, "Speedometer pos Y", {"spY"}, "", 1, 1000, 888, 1, function(y)
	sp_posY=y/1000
end)
menu.action(nfs,"Default speedometer pos", {"defsp"}, "",function()
	sp_posX = 0.93
	sp_posY = 0.88
end)
end
