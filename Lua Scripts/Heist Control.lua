    ...
    for _, lang_dir in pairs(LangDirs) do
        if dir == lang_dir then
            if filesystem.exists(lang_dir) then
                ...
                ERROR_LOG(DIR_TO_FILE_NAME("Language", lang_dir) ..
                " language file for HC doesn't exist." ..
...
function IS_WORKING(is_add_new_line)
    ...
    if util.is_session_started() then                                                       -- Because unable to get local variable in story mode
        if GET_INT_LOCAL("freemode", 3775) ~= util.joaat("lr_prop_carkey_fob") then         -- freemode.c, joaat("lr_prop_carkey_fob")
            ...
            "\n" ..
            TRANSLATE("- This feature isn't working due to the latest GTA Online patch:") ..
            " " ..
            Version ..
            ", " ..
            TRANSLATE(
            "Please download the latest version of Heist Control or wait for Heist Control's developer patching.")
...
function NOTIFY(Message)
    ...
    elseif READ_SETTING("Notification Type") == "In-Game" then
        if util.is_session_started() then         -- Credit goes to WiriScript
            ...
            HUD.END_TEXT_COMMAND_THEFEED_POST_MESSAGETEXT(Icon, Icon, true, 1, TRANSLATE("Heist Control"),
                "~c~" .. TRANSLATE("Notification"))
...
if not filesystem.exists(FileDirs.Native) then
    ...
    "\n\n" ..
    TRANSLATE(
    "Please re-enable 'Stand > Lua Scripts > Repository > natives-1681379138' or please join HC DC server to get support!"))
end
...
PERICO_HEIST = menu.list(menu.my_root(), TRANSLATE("Cayo Perico Heist"), { "hccp" },
    TRANSLATE("Max payout for this heist") ..
    "\n\n" ..
...
CASINO_HEIST = menu.list(menu.my_root(), TRANSLATE("Diamond Casino Heist"), { "hccah" },
    TRANSLATE("Max payout for this heist") ..
    "\n\n" ..
    TRANSLATE("- Under $3.650.000 per run") ..
    "\n\n" .. TRANSLATE("You won't get money if you don't keep money limitation!"), function() ; end)
DOOMS_HEIST = menu.list(menu.my_root(), TRANSLATE("Doomsday Heist"), { "hcdooms" },
    TRANSLATE("Max payout for this heist") ..
    "\n\n" ..
    TRANSLATE("- Under $2.550.000 per run") ..
    "\n\n" .. TRANSLATE("You won't get money if you don't keep money limitation!"), function() ; end)
CLASSIC_HEISTS = menu.list(menu.my_root(), TRANSLATE("Classic Heist"), { "hcclassic" },
    TRANSLATE("Max payout for this heist") ..
    "\n\n" ..
...
CAYO_PRESETS = menu.list(PERICO_HEIST, TRANSLATE("Automated Presets"), {},
    TRANSLATE("You should enable the preset until the end of the heist!") ..
    "\n\n" ..
    TRANSLATE(
    "Enabling one of these presets will complete all of setups, and set as max payout, $2.55 Millions. It will be affected to all of heist players."),
    function() ; end)
...
QUICK_PRESET = menu.toggle(CAYO_PRESETS, TRANSLATE("Quick Preset (1 - 4P)"), { "hccpquick" },
    IS_WORKING(true) ..
    TRANSLATE(
    "There is only a primary target, depends on which you selected. All players of the heist session can get the max payout ($2.45M) by only getting it."),
    function()
        if menu.get_value(QUICK_PRESET) then
            ...
            for i = 1, 5 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        TRANSLATE("This feature has been controlled by another feature.") ..
...
QUICK_PRESET_TARGET = menu.list_select(CAYO_PRESETS, TRANSLATE("Select Primary Target"), {},
    "(" .. menu.get_menu_name(QUICK_PRESET) .. ")", {
...
AUTOAMTED_SOLO_SAPPHIRE = menu.toggle(AUTOMATED_SOLO, TRANSLATE("Sapphire Panther"), { "hccp1psp" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 3 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        "\n" ..
                        "- " ..
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_SOLO_SAPPHIRE)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_SOLO_SAPPHIRE))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOMATED_SOLO_RUBY = menu.toggle(AUTOMATED_SOLO, TRANSLATE("Ruby Necklace"), { "hccp1prn" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 3 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOMATED_SOLO_RUBY)) ..
                        " > " .. menu.get_menu_name(AUTOMATED_SOLO_RUBY))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOAMTED_2P_SAPPHIRE = menu.toggle_loop(AUTOMATED_2P, TRANSLATE("Sapphire Panther"), { "hccp2psp" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 4 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_2P_SAPPHIRE)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_2P_SAPPHIRE))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOAMTED_2P_RUBY = menu.toggle_loop(AUTOMATED_2P, TRANSLATE("Ruby Necklace"), { "hccp2prn" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 4 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_2P_RUBY)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_2P_RUBY))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOAMTED_3P_SAPPHIRE = menu.toggle_loop(AUTOMATED_3P, TRANSLATE("Sapphire Panther"), { "hccp3psp" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 5 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_3P_SAPPHIRE)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_3P_SAPPHIRE))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOAMTED_3P_RUBY = menu.toggle_loop(AUTOMATED_3P, TRANSLATE("Ruby Necklace"), { "hccp3prn" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 5 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_3P_RUBY)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_3P_RUBY))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOAMTED_4P_SAPPHIRE = menu.toggle_loop(AUTOMATED_4P, TRANSLATE("Sapphire Panther"), { "hccp4psp" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 6 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_4P_SAPPHIRE)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_4P_SAPPHIRE))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
AUTOAMTED_4P_RUBY = menu.toggle_loop(AUTOMATED_4P, TRANSLATE("Ruby Necklace"), { "hccp4prn" }, IS_WORKING(false),
    function()
            ...
            for i = 1, 6 do
                for j = 1, #ForCayoPresets[i] do
                    menu.set_help_text(ForCayoPresets[i][j][1],
                        ...
                        menu.get_menu_name(CAYO_PRESETS) ..
                        " > " ..
                        menu.get_menu_name(menu.get_parent(AUTOAMTED_4P_RUBY)) ..
                        " > " .. menu.get_menu_name(AUTOAMTED_4P_RUBY))
                end
            end
        else
            for i = 1, #CayoPresetLists do
...
TELEPORT_CP = menu.list(PERICO_HEIST, TRANSLATE("Teleport Places"), {},
    TRANSLATE("- How to change the line color: Stand > Settings > Appearance > Colours > HUD Colour") ..
    "\n\n" .. TRANSLATE("- How to change the AR Beacon color: Stand > Settings > Appearance > Colours > AR Colour"),
    function() ; end)
...
TELEPORT_CP_KOSATKA = menu.action(TELEPORT_CP, TRANSLATE("Kosatka: Heist Board"), { "hctpsub" },
    TRANSLATE("Note that works on best when you are alone in your session."), function()
...
PERICO_CUTS = menu.list(PERICO_ADV, TRANSLATE("Player's Cut"), {},
    TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."),
    function() ; end)

PERICO_NON_HOST_CUT = menu.list(PERICO_CUTS, TRANSLATE("Your Cut (Non-Host)"), {},
    TRANSLATE(
    "Note that modifying this won't applied to the heist board. You can check the applied cut after end of the heist."),
    function() ; end)

CP_NON_HOST_CUT_LOOP = menu.toggle_loop(PERICO_NON_HOST_CUT, TRANSLATE("Enable"), { "hccpnonhostloop" },
    IS_WORKING(false), function()
...
CP_NON_HOST_CUT = menu.slider(PERICO_NON_HOST_CUT, TRANSLATE("Custom Percentage"), { "hccpnonhost" }, "(%)", 0, 1000, 100,
    5, function() ; end)

---

PERICO_HOST_CUT = menu.list(PERICO_CUTS, TRANSLATE("Your Cut"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)

CP_HOST_CUT_LOOP = menu.toggle_loop(PERICO_HOST_CUT, TRANSLATE("Enable"), { "hccphostcutloop" }, IS_WORKING(false),
    function()
...
CP_HOST_CUT = menu.slider(PERICO_HOST_CUT, TRANSLATE("Custom Percentage"), { "hccphostcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

PERICO_P2_CUT = menu.list(PERICO_CUTS, TRANSLATE("Player 2"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CP_2P_CUT = menu.slider(PERICO_P2_CUT, TRANSLATE("Custom Percentage"), { "hccp2pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

PERICO_P3_CUT = menu.list(PERICO_CUTS, TRANSLATE("Player 3"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CP_3P_CUT = menu.slider(PERICO_P3_CUT, TRANSLATE("Custom Percentage"), { "hccp3pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

PERICO_P4_CUT = menu.list(PERICO_CUTS, TRANSLATE("Player 4"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CP_4P_CUT = menu.slider(PERICO_P4_CUT, TRANSLATE("Custom Percentage"), { "hccp4pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)
...
menu.toggle_loop(PERICO_ADV, TRANSLATE("Skip The Hacking Process"), {},
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " ..
    TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Compound") .. " > " .. TRANSLATE("El Rubio's Office") .. ")",
    function()
...
menu.toggle_loop(PERICO_ADV, TRANSLATE("Skip Cutting The Sewer Grill"), {},
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " .. TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Island") .. " > " .. TRANSLATE("Drainage Pipe") .. ")",
    function()
...
menu.toggle_loop(PERICO_ADV, TRANSLATE("Skip Cutting The Glass"), {},
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " ..
    TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Compound") .. " > " .. TRANSLATE("Primary Target") .. ")",
    function()
...
menu.toggle_loop(PERICO_ADV, TRANSLATE("Infinite Plasma Cutter Heat"), {},
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " ..
...
menu.toggle_loop(PERICO_ADV, TRANSLATE("Infinite Voltage Timer"), {},
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " .. TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Island") .. " > " .. TRANSLATE("Radio Tower") .. ")",
    function()
...
CP_REM_FEE = menu.toggle_loop(PERICO_ADV, TRANSLATE("Remove The Fencing Fee And Pavel's Cut"), { "hccpremfee" },
...
menu.action(PERICO_ADV, TRANSLATE("Obtain The Primary Target"), { "hccpfintar" },
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " ..
    TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Compound") .. " > " .. TRANSLATE("Primary Target") .. ")",
    function()                                                                                                                                                                                                                                                                          -- https://www.unknowncheats.me/forum/3418914-post13398.html
        SET_INT_LOCAL("fm_mission_controller_2020", 30356, 5)                                                                                                                                                                                                                           -- CutterStage
        SET_INT_LOCAL("fm_mission_controller_2020", 30357, 3)                                                                                                                                                                                                                           -- BitCheck
    end)

menu.action(PERICO_ADV, TRANSLATE("Remove The Drainage Pipe"), { "hccprempipe" },
    "(" ..
    TRANSLATE("Cayo Perico Heist") ..
    " > " .. TRANSLATE("Teleport Places") .. " > " .. TRANSLATE("Island") .. " > " .. TRANSLATE("Drainage Pipe") .. ")",
    function()
...
menu.action(PERICO_ADV, TRANSLATE("Refresh Kosatka Planning Table"), { "hccprefreshboard" },
    IS_WORKING(true) .. TRANSLATE("You can update changed cayo perico heist stats in the Kosatka by refreshing it."),
    function()
...
menu.list_action(PERICO_HEIST, TRANSLATE("Secondary Target"), { "hccpsectar" },
    TRANSLATE("When using this method, the percentage and final payment will be random!"), {
...
CASINO_PRESETS = menu.list(CASINO_HEIST, TRANSLATE("Automated Presets"), {},
    TRANSLATE("Please don't forget that buyer should be selected 'Low'! Otherwise, won't get money."), function() ; end)

CASINO_PRESETS_DIAMOND = menu.list(CASINO_PRESETS, TRANSLATE("Diamonds"), {},
    TRANSLATE("If the Heist Planning (Board 3) is empty, then please re-enable the preset that you enabled!"),
    function() ; end)

DIAMOND_SS = menu.toggle(CASINO_PRESETS_DIAMOND, TRANSLATE("Silent & Sneaky Approach"), { "hccahdiasil" },
    IS_WORKING(false), function()
    if menu.get_value(DIAMOND_SS) then
        ...
        for i = 1, #ForCasinoPresets do
            menu.set_help_text(ForCasinoPresets[i][1],
                TRANSLATE("This feature has been controlled by another feature.") ..
                "\n" ..
                "- " ..
                menu.get_menu_name(CASINO_PRESETS) ..
                " > " .. menu.get_menu_name(menu.get_parent(DIAMOND_SS)) .. " > " .. menu.get_menu_name(DIAMOND_SS))
        end
    else
        for i = 1, #CasinoPresetLists do
            if CasinoPresetLists[i][1] == DIAMOND_SS then
...
DIAMOND_BA = menu.toggle(CASINO_PRESETS_DIAMOND, TRANSLATE("BigCon Approach"), { "hccahdiabig" }, IS_WORKING(false),
    function()
        if menu.get_value(DIAMOND_BA) then
            ...
            for i = 1, #ForCasinoPresets do
                menu.set_help_text(ForCasinoPresets[i][1],
                    ...
                    "- " ..
                    menu.get_menu_name(CASINO_PRESETS) ..
                    " > " .. menu.get_menu_name(menu.get_parent(DIAMOND_BA)) .. " > " .. menu.get_menu_name(DIAMOND_BA))
            end
        else
            for i = 1, #CasinoPresetLists do
                if CasinoPresetLists[i][1] == DIAMOND_BA then
...
DIAMOND_AA = menu.toggle(CASINO_PRESETS_DIAMOND, TRANSLATE("Aggressive Approach"), { "hccahdiaagg" }, IS_WORKING(false),
    function()
        if menu.get_value(DIAMOND_AA) then
            ...
            for i = 1, #ForCasinoPresets do
                menu.set_help_text(ForCasinoPresets[i][1],
                    ...
                    "- " ..
                    menu.get_menu_name(CASINO_PRESETS) ..
                    " > " .. menu.get_menu_name(menu.get_parent(DIAMOND_AA)) .. " > " .. menu.get_menu_name(DIAMOND_AA))
            end
        else
            for i = 1, #CasinoPresetLists do
                if CasinoPresetLists[i][1] == DIAMOND_AA then
...
CASINO_PRESETS_GOLD = menu.list(CASINO_PRESETS, TRANSLATE("Gold"), {},
    TRANSLATE("If the Heist Planning (Board 3) is empty, then please re-enable the preset that you enabled!"),
    function() ; end)

GOLD_SS = menu.toggle(CASINO_PRESETS_GOLD, TRANSLATE("Silent & Sneaky Approach"), { "hccahgoldsil" }, IS_WORKING(false),
    function()
        if menu.get_value(GOLD_SS) then
            ...
            for i = 1, #ForCasinoPresets do
                menu.set_help_text(ForCasinoPresets[i][1],
                    ...
                    "- " ..
                    menu.get_menu_name(CASINO_PRESETS) ..
                    " > " .. menu.get_menu_name(menu.get_parent(GOLD_SS)) .. " > " .. menu.get_menu_name(GOLD_SS))
            end
        else
            for i = 1, #CasinoPresetLists do
                if CasinoPresetLists[i][1] == GOLD_SS then
...
GOLD_BA = menu.toggle(CASINO_PRESETS_GOLD, TRANSLATE("BigCon Approach"), { "hccahgoldbig" }, IS_WORKING(false),
    function()
        if menu.get_value(GOLD_BA) then
            ...
            for i = 1, #ForCasinoPresets do
                menu.set_help_text(ForCasinoPresets[i][1],
                    ...
                    "- " ..
                    menu.get_menu_name(CASINO_PRESETS) ..
                    " > " .. menu.get_menu_name(menu.get_parent(GOLD_BA)) .. " > " .. menu.get_menu_name(GOLD_BA))
            end
        else
            for i = 1, #CasinoPresetLists do
                if CasinoPresetLists[i][1] == GOLD_BA then
...
GOLD_AA = menu.toggle(CASINO_PRESETS_GOLD, TRANSLATE("Aggressive Approach"), { "hccahgoldagg" }, IS_WORKING(false),
    function()
        if menu.get_value(GOLD_AA) then
            ...
            for i = 1, #ForCasinoPresets do
                menu.set_help_text(ForCasinoPresets[i][1],
                    ...
                    "- " ..
                    menu.get_menu_name(CASINO_PRESETS) ..
                    " > " .. menu.get_menu_name(menu.get_parent(GOLD_AA)) .. " > " .. menu.get_menu_name(GOLD_AA))
            end
        else
            for i = 1, #CasinoPresetLists do
                if CasinoPresetLists[i][1] == GOLD_AA then
...
CAH_PLAYER_CUT = menu.list(CAH_ADVCED, TRANSLATE("Player's Cut"), {},
    TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."),
    function() ; end)

CAH_NON_HOST = menu.list(CAH_PLAYER_CUT, TRANSLATE("Your Cut (Non-Host)"), {},
    TRANSLATE(
    "Note that modifying this won't applied to the heist board. You can check the applied cut after end of the heist."),
    function() ; end)

CAH_NON_HOST_CUT_LOOP = menu.toggle_loop(CAH_NON_HOST, TRANSLATE("Enable"), { "hccahnonhostloop" }, IS_WORKING(false),
    function()
...
CAH_NON_HOST_CUT = menu.slider(CAH_NON_HOST, TRANSLATE("Custom Percentage"), { "hccahnonhost" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

CAH_HOST = menu.list(CAH_PLAYER_CUT, TRANSLATE("Your Cut"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CAH_HOST_CUT = menu.slider(CAH_HOST, TRANSLATE("Custom Percentage"), { "hccah1pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

CAH_2P = menu.list(CAH_PLAYER_CUT, TRANSLATE("Player 2"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CAH_2P_CUT = menu.slider(CAH_2P, TRANSLATE("Custom Percentage"), { "hccah2pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

CAH_3P = menu.list(CAH_PLAYER_CUT, TRANSLATE("Player 3"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CAH_3P_CUT = menu.slider(CAH_3P, TRANSLATE("Custom Percentage"), { "hccah3pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

CAH_4P = menu.list(CAH_PLAYER_CUT, TRANSLATE("Player 4"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
CAH_4P_CUT = menu.slider(CAH_4P, TRANSLATE("Custom Percentage"), { "hccah4pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)
...
menu.toggle_loop(CAH_ADVCED, TRANSLATE("Skip The Hacking Process"), {},
    IS_WORKING(true) .. TRANSLATE("Works On Both: Fingerprint and Keypad"), function()
...
menu.action(CAH_ADVCED, TRANSLATE("Skip Drilling The Vault Door"), { "hccahinsvault" }, IS_WORKING(false),
    function()                                                                                                              -- https://www.unknowncheats.me/forum/3418914-post13398.html
        SET_INT_LOCAL("fm_mission_controller", 10107 + 7, GET_INT_LOCAL("fm_mission_controller", 10107 + 37))
    end)

menu.action(CAH_ADVCED, TRANSLATE("Refresh Arcade Boards"), { "hccahrefreshboards" },
    TRANSLATE("You can update casino heist stats while even you in the arcade."), function()
...
TELEPORT_CAH = menu.list(CASINO_HEIST, TRANSLATE("Teleport Places"), {},
    TRANSLATE("- How to change the line color: Stand > Settings > Appearance > Colours > HUD Colour") ..
    "\n\n" .. TRANSLATE("- How to change the AR Beacon color: Stand > Settings > Appearance > Colours > AR Colour"),
    function() ; end)
...
TELEPORT_CAH_ENTRANCE = menu.action(TELEPORT_CAH, TRANSLATE("Entrance Of Your Arcade"), {},
    TRANSLATE("Note that works perfectly when no other players are in your session."), function()
...
TELEPORT_CAH_IN_BOARD = menu.action(TELEPORT_CAH, TRANSLATE("Planning Boards (Arcade)"), {},
    TRANSLATE("You should in your arcade to use this teleport!"), function()
...
TELEPORT_CAH_IN_EXIT = menu.action(TELEPORT_CAH, TRANSLATE("Garage Exit (Arcade)"), {},
    TRANSLATE("You should in your arcade to use this teleport!"), function()
    if IS_IN_ARCADE() then
...
menu.slider(CASINO_BOARD2, TRANSLATE("Guards Strength Level"), { "hccahguard" },
    TRANSLATE("0 - Strongest Guards") .. "\n" .. TRANSLATE("3 - Weakest Guards"), 0, 3, 3, 1, function(value)
...
menu.list_action(CASINO_BOARD3, TRANSLATE("Entrance"), { "hccahentrance" },
    IS_WORKING(true) ..
    TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."), {
...
menu.list_action(CASINO_BOARD3, TRANSLATE("Exit"), { "hccahexit" },
    IS_WORKING(true) ..
    TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."), {
    { 1,  TRANSLATE("Main Door"),        { "maindoor" },     "" },
    { 2,  TRANSLATE("Staff Lobby"),      { "stafflobby" },   "" },
...
menu.list_action(CASINO_BOARD3, TRANSLATE("Buyer Level"), { "hccahbuyer" },
    IS_WORKING(true) ..
    TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."), {
    { 1, TRANSLATE("Low"),  { "low" }, "" },
    { 2, TRANSLATE("Mid"),  { "mid" }, "" },
...
menu.list_action(CASINO_BOARD3, TRANSLATE("Entry Disguise"), { "hccahentrydisguise" },
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("BigCon Approach") ..
    ")" ..
    "\n\n" .. TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."), {
...
menu.list_action(CASINO_BOARD3, TRANSLATE("Exit Disguise"), { "hccahexitdisguise" },
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("BigCon Approach") ..
    ")" ..
...
REM_REPEAT_COOLDOWN = menu.action(CASINO_MORE, TRANSLATE("Remove Repeat Cooldown"), {},
    TRANSLATE("You should not in your arcade to use this feature.") ..
...
DOOMS_PRESETS = menu.list(DOOMS_HEIST, TRANSLATE("Automated Presets"), { "hcdoomspreset" },
    TRANSLATE("You should enable the preset until the end of the heist!") ..
    "\n\n" ..
    TRANSLATE(
    "Enabling one of these presets will complete all of setups, and set as max payout, $2.55 Millions. It will be affected to all of heist players."),
...
DOOMS_PRESETS_ACT1 = menu.toggle(DOOMS_PRESETS, TRANSLATE("ACT I - The Data Breaches"), { "hcdoomsact1" },
    IS_WORKING(true) .. TRANSLATE("Note that there is a payout visual bug on the heist screen."), function()
    if menu.get_value(DOOMS_PRESETS_ACT1) then
        ...
        for i = 1, #ForDoomsPresets do
            menu.set_help_text(ForDoomsPresets[i][1],
                TRANSLATE("This feature has been controlled by another feature.") ..
...
DOOMS_PRESETS_ACT2 = menu.toggle(DOOMS_PRESETS, TRANSLATE("ACT II - The Bogdan Problem"), { "hcdoomsact2" },
    IS_WORKING(true) .. TRANSLATE("Note that there is a payout visual bug on the heist screen."), function()
...
DOOMS_PRESETS_ACT3 = menu.toggle(DOOMS_PRESETS, TRANSLATE("ACT III - The Doomsday Scenario"), { "hcdoomsact3" },
    IS_WORKING(true) .. TRANSLATE("Note that there is a payout visual bug on the heist screen."), function()
...
TELEPORT_DOOMS = menu.list(DOOMS_HEIST, TRANSLATE("Teleport Places"), {},
    TRANSLATE("- How to change the line color: Stand > Settings > Appearance > Colours > HUD Colour") ..
    "\n\n" .. TRANSLATE("- How to change the AR Beacon color: Stand > Settings > Appearance > Colours > AR Colour"),
    function() ; end)
...
TELEPORT_DOOMS_ENTRANCE = menu.action(TELEPORT_DOOMS, TRANSLATE("Entrance Of Your Facility"), {},
    TRANSLATE("Note that works perfectly when no other players are in your session."), function()
...
TELEPORT_DOOMS_SCREEN = menu.action(TELEPORT_DOOMS, TRANSLATE("Heist Screen on Facility"), {},
    TRANSLATE("Make sure you are in your facility."), function()
...
TELEPORT_DOOMS_BOARD = menu.action(TELEPORT_DOOMS, TRANSLATE("Heist Board"), {},
    "(" .. TRANSLATE("The Bogdan Problem ACT II") .. ")", function()
    TELEPORT(515.528, 4835.353, -62.587)
end)
TELEPORT_DOOMS_CELL = menu.action(TELEPORT_DOOMS, TRANSLATE("Prisoner Cell"), {},
    "(" .. TRANSLATE("The Bogdan Problem ACT II") .. ")", function()
...
DOOMS_PLAYER_CUT = menu.list(DOOMS_HEIST, TRANSLATE("Player's Cut"), {},
    TRANSLATE("Don't forget changing this feature before starting the heist! Otherwise, won't be applied."),
    function() ; end)

DOOMS_NON_HOST = menu.list(DOOMS_PLAYER_CUT, TRANSLATE("Your Cut (Non-Host)"), {},
    TRANSLATE(
    "Note that modifying this won't applied to the heist board. You can check the applied cut after end of the heist."),
    function() ; end)

DOOMS_NON_HOST_CUT_LOOP = menu.toggle_loop(DOOMS_NON_HOST, TRANSLATE("Enable"), { "hcdoomsnonhostloop" },
    IS_WORKING(false), function()
...
DOOMS_NON_HOST_CUT = menu.slider(DOOMS_NON_HOST, TRANSLATE("Custom Percentage"), { "hcdoomsnonhost" }, "(%)", 0, 1000,
    100, 5, function() ; end)

---

DOOMS_HOST = menu.list(DOOMS_PLAYER_CUT, TRANSLATE("Your Cut"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)

DOOMS_HOST_CUT_LOOP = menu.toggle_loop(DOOMS_HOST, TRANSLATE("Enable"), { "hcdooms1ploop" }, IS_WORKING(false),
    function()
...
DOOMS_HOST_CUT = menu.slider(DOOMS_HOST, TRANSLATE("Custom Percentage"), { "hcdooms1pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

DOOMS_2P = menu.list(DOOMS_PLAYER_CUT, TRANSLATE("Player 2"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
DOOMS_2P_CUT = menu.slider(DOOMS_2P, TRANSLATE("Custom Percentage"), { "hcdooms2pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

DOOMS_3P = menu.list(DOOMS_PLAYER_CUT, TRANSLATE("Player 3"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
DOOMS_3P_CUT = menu.slider(DOOMS_3P, TRANSLATE("Custom Percentage"), { "hcdooms3pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)

---

DOOMS_4P = menu.list(DOOMS_PLAYER_CUT, TRANSLATE("Player 4"), {}, TRANSLATE("Only works if you are host of the heist."),
    function() ; end)
...
DOOMS_4P_CUT = menu.slider(DOOMS_4P, TRANSLATE("Custom Percentage"), { "hcdooms4pcut" }, "(%)", 0, 1000, 100, 5,
    function() ; end)
...
menu.toggle_loop(DOOMS_PLAYER_CUT, TRANSLATE("Bypass Minimum And Maximum Percentage"), {},
    TRANSLATE("Only works if you are host of the heist.") ..
    "\n\n" ..
...
menu.toggle_loop(DOOMS_HEIST, TRANSLATE("Skip The Hacking Process"), {},
    IS_WORKING(true) ..
    "(" ..
    TRANSLATE("The Data Breaches ACT I") ..
    " - " .. TRANSLATE("Setup: Server Farm (Lester)") .. " & " .. TRANSLATE("The Doomsday Scenario ACT III") .. ")",
    function()
...
menu.action(DOOMS_HEIST, TRANSLATE("Refresh Heist Screen On Facility"), { "hcdoomsrefreshscreen" },
    IS_WORKING(true) .. TRANSLATE("You can update changed doomsday heist stats in the Facility by refreshing it."),
    function()
...
menu.action(DOOMS_HEIST, TRANSLATE("Unlock All Doomsday Heists"), {},
    TRANSLATE("Makes able to play all of Doomsday heists, ACT I, II, III."), function()
...
menu.toggle_loop(FLEECA_HEIST, TRANSLATE("Set Your Payout To $15M"), { "hcfleeca15m" },
    IS_WORKING(true) ..
    TRANSLATE("(Local)") ..
...
menu.toggle_loop(FLEECA_HEIST, TRANSLATE("Skip The Hacking Process"), {}, IS_WORKING(false),
    function()                                                                                                  -- https://www.unknowncheats.me/forum/3455828-post8.html
        SET_INT_LOCAL("fm_mission_controller", 11776 + 24, 7)
    end)

menu.toggle_loop(FLEECA_HEIST, TRANSLATE("Skip Drilling"), {}, IS_WORKING(false),
    function()                                                                                       -- https://www.unknowncheats.me/forum/3485435-post19.html
...
menu.toggle_loop(PACIFIC_STANDARD_HEIST, TRANSLATE("Keep Cash $1.85M Take"), {},
    IS_WORKING(true) ..
    TRANSLATE("This makes you won't lose money by getting shot to the cash bags from polices") ..
    "\n\n" ..
    TRANSLATE(
    "Note that you shouldn't enable this feature while playing the Casino Heist: Because due to synced with payout for it"),
    function()
        SET_INT_LOCAL("fm_mission_controller", 19728 + 2686, 1850000)                                                     -- How much did you take in the casino and pacific standard heist
    end)
menu.toggle_loop(PACIFIC_STANDARD_HEIST, TRANSLATE("Skip The Hacking Process"), {}, IS_WORKING(false),
    function()                                                                                                            -- https://www.unknowncheats.me/forum/3694259-post117.html
...
menu.toggle_loop(CLASSIC_HEISTS, TRANSLATE("Bypass Minimum And Maximum Percentage"), {},
    TRANSLATE("Only works if you are host of the heist.") ..
    "\n\n" ..
    TRANSLATE("It allows you set 0 ~ 100(%) by modifying the heist board, ignores all of restrictions modifying cuts."),
    function()
...
menu.toggle_loop(CLASSIC_HEISTS, TRANSLATE("Complete All Setup"), {},
    TRANSLATE("Works on all of the classic heists. You need to activate this until first setup mission is ended."),
    function()
...
menu.toggle_loop(LS_ROBBERY, TRANSLATE("Modify Contracts payout - $1 Million"), { "hcls1m" },
    IS_WORKING(true) ..
...
menu.toggle_loop(ULP_MISSIONS, TRANSLATE("Skip The Hacking Process"), {},
    IS_WORKING(true) .. "(" .. TRANSLATE("ULP Missions") .. " - " .. TRANSLATE("Counterintelligence") .. ")",
    function()                                                                                                                                                                                         -- Thanks for coding this, Pedro9558#3559
...
menu.toggle_loop(TH_CONTRACT, TRANSLATE("Modify Finale's Payout (2 Million)"), { "hcagc2m" }, TRANSLATE("(Local)"),
    function()
...
menu.toggle_loop(TH_CONTRACT, TRANSLATE("Remove Contracts & Payphone Hits Cooldown"), { "hcagcremcontractcool" },
    TRANSLATE("Make sure it's enabled before starting any contracts or hits."),
    function()                                                                                                                                                                                                 -- Credit goes to Da Chaos#9262
...
menu.textslider(TH_CONTRACT, TRANSLATE("Remote Access: Agency App"), { "hcappagency" },
    TRANSLATE("Note that you don't have the app, some of functions won't work."), {
    TRANSLATE("Open"),
    TRANSLATE("Close"),
}, function()
...
menu.action(DRUG_WAR, TRANSLATE("Unlock Clothing"), {}, "",
    function()                                                                     -- https://www.unknowncheats.me/forum/3635453-post69.html
...
menu.action(UNLOCKER_BUILDING, TRANSLATE("Skip Nightclub Setups"), {},
    TRANSLATE("Skip all of setups when you buy a Nightclub first: Equipment, Staff, and Collect DJ") ..
    "\n\n" .. TRANSLATE("Change your session to apply!"),
    function()                                                                                                                                                                                                                                      -- https://www.unknowncheats.me/forum/3735297-post3416.html
        SET_PACKED_STAT_BOOL_CODE(18161, true)
        SET_PACKED_STAT_BOOL_CODE(22067, true)
        SET_PACKED_STAT_BOOL_CODE(22068, true)
    end)

menu.action(UNLOCKER_BUILDING, TRANSLATE("Acid Lab Equipment Upgrade"), {}, "",
    function()                                                                                     -- https://www.unknowncheats.me/forum/3631045-post2754.html
...
menu.action(UNLOCKER_BUILDING, TRANSLATE("Unlock Arcade Awards"), {},
    TRANSLATE("Trophies, toys, and clothings are included."), function()
...
menu.action(UNLOCKER_BUILDING, TRANSLATE("Casino Store Ace Masks"), {},
    TRANSLATE("Make sure click before buying. When you change your session, will be changed to non-unlocked status."),
    function()
        SET_PACKED_INT_TUNABLE_GLOBAL("VC_ACE_OF_SPADES", "VC_ACE_OF_DIAMONDS", 1)
    end)

menu.action(UNLOCKER_BUILDING, TRANSLATE("Add Cosmetic Decorations To The Office/MC"), {},
    TRANSLATE("To apply, sell special crate or vehicle cargo and change your session!"), function()
...
UNLOCKER_HEISTS = menu.list(MASTER_UNLOCKER, TRANSLATE("Heists"), {},
    TRANSLATE("Unlocks almost of unlockable stuffs related the heist like clothing, etc."), function() ; end)
...
menu.action(UNLOCKER_MISSIONS, TRANSLATE("Unlock Taxi Missions"), {}, "",
    function()                                                                               -- https://github.com/ImSapphire/unlock_drug_war_content/blob/main/unlock_drug_war_content.lua
...
menu.action(UNLOCKER_MISSIONS, TRANSLATE("Skip Lamar Missions To The Last One"), {},
    TRANSLATE("Change your session to apply!"), function()                                                                                          -- https://www.unknowncheats.me/forum/2770402-post3008.html
...
menu.toggle_loop(XMAS_FEATURES, TRANSLATE("Bypass Christmas Clothing"), {},
    TRANSLATE("You must keep this feature enabled in order to wear it!"), function()
...
menu.toggle_loop(UNLOCKER_ANNIVERSARY, TRANSLATE("Independence's Day"), {},
    TRANSLATE("Note: You may need to keep activating to use some of Independence Day's contents."), function()
...
menu.action(UNLOCKER_SHIRT_HAT, TRANSLATE("Unlock Lots of Hats And Shirts"), {},
    TRANSLATE("500+ unlocks are included, try and see how many clothes would be unlocked."), function()
...
menu.action(UNLOCKER_SHIRT_HAT, TRANSLATE("Unlock Sasquatch Outfit"), {}, "",
    function()                                                                                   -- https://www.unknowncheats.me/forum/3492512-post53.html
...
menu.action(UNLOCKER_TATTOO, TRANSLATE("Alien Tatto (Illuminati)"), {}, TRANSLATE("Change your session to apply!"),
    function()
...
menu.action(UNLOCKER_VEHICLE, TRANSLATE("Taxi Livery"), {}, TRANSLATE("a.k.a: 'Downtown Cab Co.' livery"),
    function()                                                                                                                -- https://www.unknowncheats.me/forum/3630202-post2734.html
...
menu.action(SPECIAL_WEAPON, TRANSLATE("Service Carbine"), {}, "",
    function()                                                                       -- https://www.unknowncheats.me/forum/3488328-post27.html
        SET_INT_TUNABLE_GLOBAL("UNLOCK_SERVICE_CARBINE_FOR_PURCHASE", 1)
    end)

menu.action(SPECIAL_WEAPON, TRANSLATE("Stone Hatchet Challenge"), {}, TRANSLATE("Change your session to apply!"),
    function()
...
menu.action(SPECIAL_WEAPON, TRANSLATE("Double Action Revolver"), {}, TRANSLATE("Change your session to apply!"),
    function()
...
menu.action(SPECIAL_WEAPON, TRANSLATE("Ceramic Pistol"), {}, "",
    function()                                                                      -- https://www.unknowncheats.me/forum/3439472-post2.html
...
menu.toggle_loop(TUNABLES_CD, TRANSLATE("Spin Lucky Wheel"), {}, "",
    function()                                                                                  -- https://www.unknowncheats.me/forum/3531489-post51.html
...
menu.toggle_loop(TUNABLES_CD, TRANSLATE("Dax Job"), {}, TRANSLATE("Note that Dax Job means from cellphone"),
    function()                                                                                                                          -- https://www.unknowncheats.me/forum/3629824-post2716.html
...
menu.toggle_loop(TUNABLES_CD, TRANSLATE("CEO Vehicles"), {}, "",
    function()                                                                              -- Credit goes to Professor#4478's Kiddions Lua
...
MUT_INPUT = menu.slider_float(TUNABLES_MUT, TRANSLATE("Multipliers"), { "hcmultiplier" }, "", 0, 500000, 100, 100,
    function() ; end)
...
TUNABLES_CAH = menu.list(TUNABLES, TRANSLATE("Casino Services: Chips") .. " " .. TRANSLATE("(Risky)"), {}, "",
    function() ; end)

CHIPS_AMOUNT = menu.slider(TUNABLES_CAH, TRANSLATE("Chips Amount"), { "hccasinochips" }, "", INT_MIN, INT_MAX, 0, 10000,
    function() ; end)
...
menu.toggle_loop(TUNABLES_CAH, TRANSLATE("Remove Cooldown Buy Chips"), {},
    TRANSLATE("If you enable this, able to buy over 20K or 50K chips by removing cooldown time."), function()
    STAT_SET_INT("MPPLY_CASINO_CHIPS_PUR_GD", 0)
    STAT_SET_INT("MPPLY_CASINO_CHIPS_PURTIM", 0)
end)

menu.action(TUNABLES_CAH, TRANSLATE("Get Visitor Bonus Again"), {}, TRANSLATE("Change your session to apply!"),
    function()
...
menu.toggle_loop(TUNABLES_OTH, TRANSLATE("Make One Snack Full Health"), {},
    TRANSLATE("Whatever you use a snack, will make you full health."), function()
    for i = 1, 7 do
...
menu.toggle_loop(TUNABLES_OTH, TRANSLATE("Infinite Stone Hatchet's Power"), {}, "",
    function()                                                                                             -- https://www.unknowncheats.me/forum/3484239-post11.html
...
menu.action(TUNABLES_OTH, TRANSLATE("Single MC Vehicle Sell"), {},
    IS_WORKING(true) ..
    TRANSLATE(
    "Forces the amount of MC Business' sale vehicles to one. Please click once before starting selling mission. Only works when you're the leader."),
    function()
        SET_INT_LOCAL("gb_biker_contraband_sell", 702 + 17, 0)         -- https://github.com/Tgamer500/YiffWarePWLua/blob/main/YiffWarePW.lua
    end)

menu.action(TUNABLES_OTH, TRANSLATE("7 Years GTA Online Playtime"), {},
    TRANSLATE("Make your account look like you've played GTA Online for 7 years."), function()
...
CUSTOM_MONEY_REMOVER = menu.slider(TUNABLES, TRANSLATE("Custom Money Remover"), { "hcmoneyremove" },
    IS_WORKING(true) .. TRANSLATE("The best way to remove GTA Online banked money up to $2B at once!"), 0, 2000000000,
    5000, 10000, function(Value)
...
menu.action(INSTANT_FINISH, TRANSLATE("Cayo / Tuners / ULP / Agency"), { "hcinsfincp" },
    IS_WORKING(true) ..
    TRANSLATE("Note that may works for some of other preps. Only 'Quick Preset' is compatible with Cayo Perico Heist."),
    function()                                                                                                                                                                                                                                         -- https://www.unknowncheats.me/forum/3524081-post3.html
...
menu.action(INSTANT_FINISH, TRANSLATE("Casino Aggressive / Classic"), { "hcinsfincah" },
    IS_WORKING(true) ..
    TRANSLATE("Note that if you don't use Heist Control's automated Casino Heist presets, won't get money."), function()
    menu.trigger_commands("scripthost")
...
menu.action(INSTANT_FINISH, TRANSLATE("Doomsday"), { "hcinsfindooms" },
    IS_WORKING(true) .. TRANSLATE("Note that you may press multiple times to instant finish the heist."), function()
    menu.trigger_commands("scripthost")
...
menu.action(INSTANT_FINISH, TRANSLATE("Bunker"), { "hcinsfinbk" }, IS_WORKING(true) .. TRANSLATE("(Selling Only)"),
    function()                                                                                                                           -- https://www.unknowncheats.me/forum/3521137-post39.html
        SET_INT_LOCAL("gb_gunrunning", 1209 + 774, 0)
    end)

menu.action(INSTANT_FINISH, TRANSLATE("Air Cargo"), { "hcinsfinac" }, IS_WORKING(true) .. TRANSLATE("(Selling Only)"),
    function()                                                                                                                              -- https://www.unknowncheats.me/forum/3513482-post37.html
        SET_INT_LOCAL("gb_smuggler", 1932 + 1035, GET_INT_LOCAL("gb_smuggler", 1932 + 1078))
    end)

menu.action(INSTANT_FINISH, TRANSLATE("Acid Lab"), { "hcinsfinacid" }, IS_WORKING(true) .. TRANSLATE("(Selling Only)"),
    function()                                                                                                                               -- https://www.unknowncheats.me/forum/3641612-post76.html
...
menu.action(INSTANT_FINISH, TRANSLATE("Headhunter"), { "hcinsfinhh" }, "",
    function()                                                                                  -- Thanks to Sapphire#6031 helping me code this
...
menu.action(INSTANT_FINISH, TRANSLATE("Sightseer"), { "hcinsfinss" }, IS_WORKING(false),
    function()                                                                                                -- https://www.unknowncheats.me/forum/3488056-post24.html
...
REMOTE_ACCESS = menu.list(TOOLS, TRANSLATE("Remote Access"), { "hcapp" },
    TRANSLATE("Let you open remotely some of apps for missions.") ..
...
menu.action(REMOTE_ACCESS, TRANSLATE("The Open Road"), { "hcapptheopenroad" },
    "(" .. TRANSLATE("Biker Business Management") .. ")", function()
...
menu.action(REMOTE_ACCESS, TRANSLATE("Touchscreen Terminal"), { "hcapptouchscreen" },
    "(" .. TRANSLATE("Terrobyte") .. ")", function()
    START_SCRIPT("CEO", "apphackertruck")
end)

---

COOLDOWN_TIMER = menu.list(TOOLS, TRANSLATE("Heist Cooldown Timers"), { "hctimer" },
    TRANSLATE(
    "There is cooldown time when after you do play any heists once. The cooldown time is 20 mins, so this let you know the time passed or not."),
    function() ; end)
...
IS_STAND_OVERLAY = menu.toggle(COOLDOWN_TIMER, TRANSLATE("Use Stand's Overlay"), {},
    TRANSLATE("Enabled: Use Stand's Overlay & Settings: Game > Info Overlay") ..
    "\n\n" .. TRANSLATE("Disabled: Use Heist Control's Overlay & Settings"), function() ; end)
CUSTOM_TIMER_TIME = menu.slider(COOLDOWN_TIMER, TRANSLATE("Set Custom Time (Seconds)"), { "hctimertime" }, "", 0, 259200,
    1200, 60, function() ; end)
...
TIMER_CAYO = menu.toggle(COOLDOWN_TIMER, TRANSLATE("Cayo Perico Heist"), { "hctimercp" }, "", function()
    ...
    while menu.get_value(TIMER_CAYO) do
        if TimerCayo.Total == 0 then
            NOTIFY(TRANSLATE("Cayo Perico Heist") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
TIMER_CASINO = menu.toggle(COOLDOWN_TIMER, TRANSLATE("Diamond Casino Heist"), { "hctimercah" }, "", function()
    ...
    while menu.get_value(TIMER_CASINO) do
        if TimerCasino.Total == 0 then
            NOTIFY(TRANSLATE("Diamond Casino Heist") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
TIMER_DOOMS = menu.toggle(COOLDOWN_TIMER, TRANSLATE("Doomsday Heist"), { "hctimerdooms" }, "", function()
    ...
    while menu.get_value(TIMER_DOOMS) do
        if TimerDooms.Total == 0 then
            NOTIFY(TRANSLATE("Doomsday Heist") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
TIMER_CLASSIC = menu.toggle(COOLDOWN_TIMER, TRANSLATE("Classic Heist"), { "hctimerclassic" }, "", function()
    ...
    while menu.get_value(TIMER_CLASSIC) do
        if TimerClassic.Total == 0 then
            NOTIFY(TRANSLATE("Classic Heist") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
TIMER_ROBBERY = menu.toggle(COOLDOWN_TIMER, TRANSLATE("LS Tuners Robbery"), { "hctimerrobbery" }, "", function()
    ...
    while menu.get_value(TIMER_ROBBERY) do
        if TimerRobbery.Total == 0 then
            NOTIFY(TRANSLATE("LS Tuners Robbery") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
TIMER_AGENCY = menu.toggle(COOLDOWN_TIMER, TRANSLATE("The Contract: Agency"), { "hctimeragency" }, "", function()
    ...
    while menu.get_value(TIMER_AGENCY) do
        if TimerAgency.Total == 0 then
            NOTIFY(TRANSLATE("The Contract: Agency") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
TIMER_CUSTOM = menu.toggle(COOLDOWN_TIMER, TRANSLATE("Custom Timer"), { "hctimercustom" }, "", function()
    if menu.get_value(TIMER_CUSTOM) then
        ...
        TimerCustom.Total, TimerCustom.Min, TimerCustom.Plus, TimerCustom.Sec = menu.get_value(CUSTOM_TIMER_TIME), 0, 0,
            0
    end

    while menu.get_value(TIMER_CUSTOM) do
        if TimerCustom.Total == 0 then
            NOTIFY(TRANSLATE("Custom Timer") ..
            " - " .. TRANSLATE("Cooldown time has passed, you can play the heist again!"))
...
util.create_tick_handler(function()
    ...
    if menu.get_value(IS_STAND_OVERLAY) then
        ...
        if IsOneOfThemEnabled then
            for i = 1, #TimerTypes do
                directx.draw_text(TimerPos.X, TimerPos.Y + 0.02 * (i - 1),
                    TimerTypes[i][2] .. " [" .. TimerTypes[i][3] .. ":" .. TimerTypes[i][4] .. "]", 2, 0.6, TimerRGB,
                    false)
            end
        end
    end
...
menu.toggle_loop(NEAR_PED_CAM, TRANSLATE("Ignore You"), { "hcignoreped" },
    TRANSLATE(
    "Makes hostile peds (Included guards, polices, and gangs) ignore you even saw you. Except for the rare case, they don't shoot to you."),
    function()
    ...
    for _, ent in pairs(entities.get_all_objects_as_handles()) do
        for __, cam in pairs(AllCamLists) do
            if ENTITY.GET_ENTITY_MODEL(ent) == cam then
                ...
                AddPos.z = AddPos.z + 1
                MISC.SHOOT_SINGLE_BULLET_BETWEEN_COORDS(AddPos.x, AddPos.y, AddPos.z, CamPos.x, CamPos.y, CamPos.z, 1000,
                    false, 0x3656C8C1, players.user_ped(), false, true, 1000)
            end
        end
    end
end)
...
IMPROVED_NO_CLIP_SPEED = menu.slider(BYPASS_DOOR, TRANSLATE("Speed of Improved No Clip"), { "hcspeednoclip" }, "", 1, 100,
    10, 5, function()
...
DISTANCE_TPTF = menu.slider(BYPASS_DOOR, TRANSLATE("Distance of Teleport To Forward"), { "hcdistptf" }, "", 1, 100, 10, 5,
    function() ; end)
...
IMPROVED_NO_CLIP = menu.toggle(BYPASS_DOOR, TRANSLATE("Improved No Clip"), { "hcnoclip" }, "",
    function()                                                                                                      -- From Command Box Scripts: No Clip
...
menu.action(BYPASS_DOOR, TRANSLATE("Teleport To Forward"), { "hctptf" }, "",
    function()                                                                                    -- Credit goes to LanceScript Reloaded
        local Handle = PED.IS_PED_IN_ANY_VEHICLE(players.user_ped(), false) and
        PED.GET_VEHICLE_PED_IS_IN(players.user_ped(), false) or players.user_ped()
...
IS_TIME_ADDING_METHOD = menu.toggle(PLAYTIME_EDITOR, TRANSLATE("Add Additional Playtime"), {},
    TRANSLATE("Enabled: Add Method") ..
    "\n" ..
    ...
    "\n\n" .. TRANSLATE("Note: The overwrite method supports up to 24.8 days, but add method supports up to 50,000 days"),
    function() ; end)
...
PLAYTIME_HOURS = menu.slider(PLAYTIME_EDITOR, TRANSLATE("Hours"), { "hcplaytimehours" }, "", 0, 50000, 0, 1,
    function() ; end)
PLAYTIME_MINS = menu.slider(PLAYTIME_EDITOR, TRANSLATE("Minutes"), { "hcplaytimemins" }, "", 0, 50000, 0, 1,
    function() ; end)
...
for i = 1, #PlaytimeStats do
    menu.action(PLAYTIME_EDITOR, PlaytimeStats[i][1], {}, "", function()
        if not menu.get_value(IS_TIME_ADDING_METHOD) then
            STAT_SET_INT(PlaytimeStats[i][2],
                menu.get_value(PLAYTIME_DAYS) * 86400000 + menu.get_value(PLAYTIME_HOURS) * 3600000 +
                menu.get_value(PLAYTIME_MINS) * 60000)
        else
            STAT_INCREMENT(PlaytimeStats[i][2],
                menu.get_value(PLAYTIME_DAYS) * 86400000 + menu.get_value(PLAYTIME_HOURS) * 3600000 +
                menu.get_value(PLAYTIME_MINS) * 60000)
        end
...
CASH_EDITOR_AMOUNT = menu.slider(CASH_EDITOR, TRANSLATE("Cash Amount"), { "hccasheditor" }, "($)", INT_MIN, INT_MAX, 0,
    10000, function() ; end)
...
for i = 0, 4 do
    ...
    CREW_LEVEL = menu.slider(CREW_LEVEL_EDITOR, TRANSLATE("Choose Crew Level"), { "hccrewlevel" .. i }, "", 1, 8000, 1, 1,
        function() ; end)
...
STAT_EDITOR = menu.list(STAT_EDITOR_READER, TRANSLATE("Custom Stat Editor") .. " " .. TRANSLATE("(Risky)"), {},
    TRANSLATE("Use at your own risk! Setting certain stats may cause a ban like casino-related stats."), function() ; end)

menu.divider(STAT_EDITOR, TRANSLATE("Modify Stat"))

STAT_EDITOR_NAME = menu.text_input(STAT_EDITOR, TRANSLATE("Stat Name"), { "hceditname" },
    TRANSLATE("Note: MP0_ or MP1_ will be selected by HC automatically, therefore don't write it."), function()
...
DATE_YEARS = menu.slider(STAT_EDITOR, TRANSLATE("Years"), { "hcedityears" }, "", 2013, os.date("%Y"), 0, 1,
    function() ; end)
...
menu.action(STAT_EDITOR, "Integer", {},
    TRANSLATE("Example Stat") ..
    "\n\n" .. TRANSLATE("Stat Name") .. ": " .. "NO_BOUGHT_YUM_SNACKS\n" .. TRANSLATE("Stat Value") .. ": " .. "30",
    function()
        if menu.get_value(STAT_EDITOR_NAME) == "" or menu.get_value(STAT_EDITOR_VALUE) == "" then
            ...
            TRANSLATE("Stat Name") ..
            ": " ..
            ADD_MP_INDEX(menu.get_value(STAT_EDITOR_NAME)) ..
            "\n" .. TRANSLATE("Stat Value") .. ": " .. menu.get_value(STAT_EDITOR_VALUE))
            FORCE_CLOUD_SAVE()
        end
    end)

menu.action(STAT_EDITOR, "Boolean", {},
    TRANSLATE("Example Stat") ..
    "\n\n" ..
    TRANSLATE("Stat Name") .. ": " .. "CL_RACE_MODDED_CAR\n" .. TRANSLATE("Stat Value") .. ": " .. "'true' or 'false'",
    function()
        if menu.get_value(STAT_EDITOR_NAME) == "" or menu.get_value(STAT_EDITOR_VALUE) == "" then
            menu.focus(STAT_EDITOR_NAME)
            NOTIFY(TRANSLATE("You didn't specify the value. Please specify it!"))
...
menu.action(STAT_EDITOR, "String", {},
    TRANSLATE("Example Stat") ..
...
menu.action(STAT_EDITOR, "Date", {},
    ...
    ": " .. TRANSLATE("Years") .. ": 1970, " .. TRANSLATE("Months") .. ": 12, " .. TRANSLATE("Days") .. ": 25",
    function()
        if menu.get_value(STAT_EDITOR_NAME) == "" then
            menu.focus(STAT_EDITOR_NAME)
            NOTIFY(TRANSLATE("You didn't specify the value. Please specify it!"))
        else
            STAT_SET_DATE(menu.get_value(STAT_EDITOR_NAME), menu.get_value(DATE_YEARS), menu.get_value(DATE_MONTHS),
                menu.get_value(DATE_DAYS), menu.get_value(DATE_HOURS), menu.get_value(DATE_MINS))
            NOTIFY(TRANSLATE("Successfully set!") ..
            "\n\n" ..
            TRANSLATE("Stat Name") ..
            ": " ..
            ...
            TRANSLATE("Years") ..
            ": " ..
            menu.get_value(DATE_YEARS) ..
            "\n" ..
            TRANSLATE("Months") ..
            ": " ..
            menu.get_value(DATE_MONTHS) ..
            "\n" ..
            TRANSLATE("Days") ..
            ": " ..
            menu.get_value(DATE_DAYS) ..
            "\n" ..
            TRANSLATE("Hours") ..
            ": " .. menu.get_value(DATE_HOURS) .. "\n" .. TRANSLATE("Mins") .. ": " .. menu.get_value(DATE_MINS))
            FORCE_CLOUD_SAVE()
        end
    end)
...
STAT_READER_NAME = menu.text_input(STAT_READER, TRANSLATE("Stat Name"), { "hcreadername" },
    TRANSLATE("Note: MP0_ or MP1_ will be selected by HC automatically, therefore don't write it."), function()
...
ClipboardString = ""
menu.toggle_loop(STAT_READER, TRANSLATE("Auto Paste To Stat Name"), {},
    TRANSLATE("If your clipboard contains stat name, the strings will be pasted into 'Stat Name' automatically."),
    function()
...
menu.action(STAT_READER, "Integer", {},
    ...
    if menu.get_value(STAT_READER_NAME) == "" then
        ...
        TRANSLATE("Stat Name") ..
        ": " ..
        ADD_MP_INDEX(menu.get_value(STAT_READER_NAME)) ..
...
menu.action(STAT_READER, "Boolean", {},
...
menu.action(STAT_READER, "String", {},
...
menu.action(STAT_READER, "Date", {},
    ...
    if menu.get_value(STAT_READER_NAME) == "" then
        ...
        "\n" ..
        "Years: " ..
        STAT_GET_DATE(menu.get_value(STAT_READER_NAME), "Years") ..
        "\n" ..
        "Months: " ..
        STAT_GET_DATE(menu.get_value(STAT_READER_NAME), "Months") ..
        "\n" ..
        "Days: " ..
        STAT_GET_DATE(menu.get_value(STAT_READER_NAME), "Days") ..
        "\n" ..
        "Hours: " ..
        STAT_GET_DATE(menu.get_value(STAT_READER_NAME), "Hours") ..
        "\n" .. "Mins: " .. STAT_GET_DATE(menu.get_value(STAT_READER_NAME), "Mins"))
        if menu.get_value(IS_READER_COPY) then
            util.copy_to_clipboard(TRANSLATE("Years") ..
            ": " ..
            STAT_GET_DATE(menu.get_value(STAT_READER_NAME), "Years") ..
            "\n" ..
            TRANSLATE("Months") ..
...
GTAHAXUI_STAT_EDITOR = menu.list(STAT_EDITOR_READER, TRANSLATE("GTAHaXUI Stat Editor"), {},
    TRANSLATE(
    "GTAHaXUI is a free mod menu for editing stats, globals, and locals from UnknownCheats. Heist Control can help you using customizable GTAHaXUI's stat txt files. Note that you can download the files by googling easily."),
    function() ; end)

GTAHAXUI_STAT_EDITOR_LIST = menu.list(GTAHAXUI_STAT_EDITOR, TRANSLATE("Load Custom Stat Files"), { "hchaxui" },
    TRANSLATE("Supported Stat Types") ..
    "\n\n" ..
    "- 'INT32': " ..
    TRANSLATE("For normal numbers, generally called 'Int(eger)'") ..
    "\n" .. "- 'INT64': " .. TRANSLATE("For more big numbers") .. "\n" .. "- 'BOOL': " .. TRANSLATE("'true' or 'false'"),
    function() ; end)

menu.divider(GTAHAXUI_STAT_EDITOR_LIST, TRANSLATE("Tools"))

menu.action(GTAHAXUI_STAT_EDITOR_LIST, TRANSLATE("Refresh"), {},
    TRANSLATE("Refresh the list via restarting Heist Control."), function()
...
for idx, file_dir in pairs(filesystem.list_files(FolderDirs.HaxUI)) do
    if string.contains(file_dir, ".txt") then
        menu.list(GTAHAXUI_STAT_EDITOR_LIST, DIR_TO_FILE_NAME("GTAHaXUI", file_dir), { "hchaxui" .. idx }, "",
            function() ; end)
...
for idx1, file_name in pairs(HaxUIFiles) do
    ...
    if HelpTexts.Type == "N/A" then
        HelpTexts.Comment = TRANSLATE(
        "Please check this file has been typed for GTAHaXUI. Applying it won't affect anything in GTAV.")
    end

    local CommandRef = menu.ref_by_command_name("hchaxui" .. idx1)
    menu.divider(CommandRef, TRANSLATE("Tools"))

    menu.action(CommandRef, TRANSLATE("Refresh"), {}, TRANSLATE("Refresh the list via restarting Heist Control."),
        function()
    ...
    menu.action(CommandRef, TRANSLATE("Apply Stats"), {}, "", function()
        if HelpTexts.Type == "N/A" then
            NOTIFY(HelpTexts.Comment)
            return
        end
        menu.show_warning(CommandRef, CLICK_MENU,
            TRANSLATE("Do you sure apply the number of stats?") .. "\n" .. "- " .. HelpTexts.MPx + HelpTexts.MPPLY,
            function()
                for i = 1, #Stats do
    ...
    menu.action(CommandRef, TRANSLATE("Delete This File"), {}, "", function()
        menu.show_warning(CommandRef, CLICK_MENU, TRANSLATE("Do you sure delete this file? It cannot be recovered!"),
            function()
    ...
    menu.action(CommandRef, TRANSLATE("Type") .. ": " .. HelpTexts.Type, {},
        TRANSLATE(
        "Except for 'N/A', doesn't matter what it is. Heist Control will make it compatible with GTAV using all methods.") ..
        "\n\n" .. TRANSLATE("'N/A' means this file isn't for setting stats for GTAHaXUI."), function() ; end)
    menu.action(CommandRef, TRANSLATE("Total Number of Lines") .. ": " .. HelpTexts.NumOfLine, {},
        TRANSLATE("It means the number of lines of the loaded file."), function() ; end)
    menu.action(CommandRef, TRANSLATE("Number of Invalid Lines") .. ": " .. HelpTexts.InvalidLine, {},
        TRANSLATE(
        "Unless this value is '0', some of stats may not be applied, but doesn't matter unless almost of total number of lines are invalid."),
        function() ; end)
    menu.action(CommandRef, TRANSLATE("Number of 'MPx' Stats") .. ": " .. HelpTexts.MPx, {},
        TRANSLATE("It means the number of 'MP0_ABCDE_FGHIJ' or 'MP1_ABCDE_FGHIJ' format stats."), function() ; end)
    menu.action(CommandRef, TRANSLATE("Number of 'MPPLY' Stats") .. ": " .. HelpTexts.MPPLY, {},
        TRANSLATE("It means the number of 'MPPLY_ABCDE_FGHIJ' format stats."), function() ; end)
...
menu.hyperlink(GTAHAXUI_STAT_EDITOR, TRANSLATE("Visit GTAHaXUI"),
    "https://www.unknowncheats.me/forum/grand-theft-auto-v/461672-gtahax-1-58-external-thread-3-a.html", "")

---

menu.hyperlink(STAT_EDITOR_READER, TRANSLATE("All Stats List"),
    "https://gist.github.com/1337Nexo/945fe9724b9dd20d33e7afeabd2746dc", "")
menu.hyperlink(STAT_EDITOR_READER, TRANSLATE("Some Useful Stats List"),
    "https://www.unknowncheats.me/forum/2770402-post3008.html", "")

---

INFO_SPOOFING = menu.list(TOOLS, TRANSLATE("Stat Spoofing"), {}, TRANSLATE("Change your session to apply!"),
    function() ; end)
...
WALLET_MONEY_SPOOFING = menu.slider(INFO_SPOOFING_WALLET_MONEY, TRANSLATE("Value"), { "hcwalletmoneyspoofing" }, "",
    INT_MIN, INT_MAX, players.get_wallet(players.user()), 10000, function() ; end)
...
TOTAL_MONEY_SPOOFING = menu.slider(INFO_SPOOFING_MONEY, TRANSLATE("Value"), { "hctotalmoneyspoofing" }, "", INT_MIN,
    INT_MAX, players.get_bank(players.user()), 10000, function() ; end)
...
KILL_SPOOFING = menu.slider(INFO_SPOOFING_KILL, TRANSLATE("Value"), { "hckillspoofing" }, "", INT_MIN, INT_MAX,
    players.get_kills(players.user()), 1, function() ; end)
...
DEATH_SPOOFING = menu.slider(INFO_SPOOFING_DEATH, TRANSLATE("Value"), { "hcdeathspoofing" }, "", INT_MIN, INT_MAX,
    players.get_deaths(players.user()), 1, function() ; end)
...
menu.toggle_loop(TOOLS_OTH, TRANSLATE("Disable Transaction Errors"), { "hcnotransactionerr" }, IS_WORKING(false),
    function()
        if IS_WORKING(false) ~= "" then return end
        if not util.is_session_started() then return end
...
menu.toggle_loop(TOOLS_OTH, TRANSLATE("Auto Become a CEO/MC"), { "hcautobecomeceomc" },
    TRANSLATE("Detects the cases that you should be a CEO/MC to start some heists/missions, make you one of it."),
    function()
        if not util.is_session_started() then return end
...
menu.toggle_loop(TOOLS_OTH, TRANSLATE("Auto Collect Targets"), {},
    TRANSLATE(
    "Collects targets via clicking left mouse button. Note that there are some delays before disabling completely."),
    function()
...
menu.textslider(TOOLS_OTH, TRANSLATE("Skips Conversation With NPCs"), {},
    TRANSLATE("Skipping white texts where are center, under of the screen.") ..
...
menu.action(TOOLS_OTH, TRANSLATE("Clear All Help Messages"), {},
    TRANSLATE("Clearing a black box with white help texts where are left, above of the screen.") ..
    "\n\n" .. TRANSLATE("Would helpful for skipping something cannot be skipped using 'Game > Skip Cutscene'"),
    function()
        HUD.CLEAR_ALL_HELP_MESSAGES()
    end)

menu.action(TOOLS_OTH, TRANSLATE("Motion Skip"), {},
    TRANSLATE("a.k.a Pause Menu Glitch") .. "\n\n" .. TRANSLATE("Can be used for getting targets slightly more fast."),
    function()
...
TRANS_FILE_NAME = menu.text_input(HC_LANG_GEN, TRANSLATE("Name of The File"), { "hcgennewtrans" },
    TRANSLATE("HC will overwrite if name of the file already exists."), function() ; end, "Custom")

menu.action(HC_LANG_GEN, TRANSLATE("Generate New Translation File"), {},
    TRANSLATE("This action will take a few seconds. Please wait for it patiently, don't press multiple times."),
    function()
        ...
        (
            ...
            TRANSLATE("Directory of the file:") ..
            " " .. "%AppData%\\Stand\\Lua Scripts\\store\\Heist Control\\Language\\" .. File
        )
    end)
...
menu.action(UPDATE_FILE_LIST, TRANSLATE("Refresh"), {}, TRANSLATE("Refresh the list via restarting Heist Control."),
    function()
...
for _, path in ipairs(filesystem.list_files(FolderDirs.Lang)) do
    if string.contains(path, ".txt") then
        menu.action(UPDATE_FILE_LIST, DIR_TO_FILE_NAME("Language", path),
            { "hcupdatetrans" .. DIR_TO_FILE_NAME("Language", path) },
            TRANSLATE("This action will take a few seconds. Please wait for it patiently, don't press multiple times."),
            function()
                    ...
                    if GeneratedTransTable[idx][1] and GeneratedTransTable[idx][2] ~= nil then
                        for __, code in pairs(GeneratedCodeTable) do
                            if GeneratedTransTable[idx][1] == code then
                                table.insert(ExistsTrans,
                                    GeneratedTransTable[idx][1] .. TransFormat .. GeneratedTransTable[idx][2])
                ...
                (
                    ...
                    TRANSLATE("- Total number of translations:") ..
                    " " .. NumberOfExistsTrans + NumberOfNotExistTrans .. "\n\n" ..
...
NOTIFICATION_SETTING = menu.list(INFOS, TRANSLATE("Notification") .. ": " .. READ_SETTING("Notification Type"),
    { "hcnotification" }, "", function() ; end)

menu.divider(NOTIFICATION_SETTING, TRANSLATE("Notification's Style"))

NOTIFICATION_ICON_SETTING = menu.list_action(NOTIFICATION_SETTING,
    TRANSLATE("Icon") .. ": " .. READ_SETTING("Notification Icon"), {}, "", {
    { 1, TRANSLATE("HC Logo"),         {}, "" },
    { 2, TRANSLATE("Lester"),          {}, "" },
...
NOTIFICATION_COLOR_SETTING = menu.list_action(NOTIFICATION_SETTING,
    TRANSLATE("Background Color") .. ": " .. READ_SETTING("Notification Color"), {}, "", {
    { 1, TRANSLATE("White"),      {}, "" },
    { 2, TRANSLATE("Black"),      {}, "" },
    { 3, TRANSLATE("Red"),        {}, "" },
    { 4, TRANSLATE("Orange"),     {}, "" },
    ...
    WRITE_SETTING("Notification Type", "In-Game")
    menu.set_menu_name(NOTIFICATION_COLOR_SETTING,
        TRANSLATE("Background Color") .. ": " .. READ_SETTING("Notification Color"))
    menu.trigger_commands("clearnotifications")
    menu.trigger_commands("clearstandnotifys")
    NOTIFY(TRANSLATE("Successfully set!"))
...
TIMER_SETTING = menu.list_action(INFOS, TRANSLATE("Heist Cooldown Timer") .. ": " .. READ_SETTING("Timer Color"),
    { "hctimercolor" },
    TRANSLATE(
    "'Stand' setting is synced with Stand's default feature: 'Stand > Settings > Appearance > Colours > AR Colour'") ..
...
menu.hyperlink(CREDITS, "jhowkNx", "https://github.com/jhowkNx",
    TRANSLATE(
    "Heist Control for 2Take1 Developer. He allowed me to use his code. HC wouldn't exist without his help. Serious thanks to him!"))
...
menu.action(CREDITS, "Leif.Erickson", {}, TRANSLATE("Maintains HC's translation") .. ": French - français",
    function() ; end)
...
menu.action(CREDITS, "Pedro9558", {}, TRANSLATE("Maintains HC's translation") .. ": Portuguese - Português",
    function() ; end)
...
menu.hyperlink(INFOS, TRANSLATE("Join Discord Server!"), "https://discord.gg/KTFAYQn5Xz",
    TRANSLATE("If you have any questions regarding Heist Control? Joining it will help you so much!") ..
    "\n\n" ..
...
menu.hyperlink(INFOS, TRANSLATE("Changelog"), "https://github.com/IceDoomfist/Stand-Heist-Control/releases",
    TRANSLATE("If you want to know what was changed in the latest version, click me!"))
...
util.create_tick_handler(function()
    ...
    if CayoEnabledPresets > 1 then
        NOTIFY(TRANSLATE("Don't enable both of automated presets at once") ..
...
util.create_tick_handler(function()
    ...
    if CasinoEnabledPresets > 1 then
        NOTIFY(TRANSLATE("Don't enable both of automated presets at once") ..
        " - " .. TRANSLATE("Diamond Casino Heist") .. "\n\n" .. TRANSLATE("Successfully cancelled!"))
        for i = 1, #CasinoPresetLists do
            if CasinoPresetLists[i][2] then
        ...
        if TPs[i][j][2] ~= nil then
            menu.on_tick_in_viewport(TPs[i][j][1], function()
                if GET_CURSOR_POSITION() == j then
                    ...
                    GRAPHICS.DRAW_LINE(PlayerPos.x, PlayerPos.y, PlayerPos.z, TPs[i][j][2].x, TPs[i][j][2].y,
                        TPs[i][j][2].z, Color.r, Color.g, Color.b, Color.a)
