-- GENERATED ON Thursday 1st, September 2022 AT 10:28:21 AM
-- AUTOGENERATED CODE --
local SCRIPT_SOURCE = "REPO"
local SCRIPT_BRANCH = "release"
local BRANCH_LAST_COMMIT = "fe34b7a51fd9bd71b72dc75cf839077f5d77159d"
...
-- END Version Check
------------------------------------------------------------------
local metaList = menu.list(menu.my_root(), "脚本支持")
menu.divider(metaList, SCRIPT .. " V" .. VERSION)
menu.hyperlink(metaList, "查看完整的更新日志", "https://jackz.me/stand/changelog?html=1&script=" .. SCRIPT)
menu.hyperlink(metaList, "杰克兹的公会", "https://www.guilded.gg/i/k8bMDR7E?cid=918b2f61-989c-41c4-ba35-8fd0e289c35d&intent=chat", "Get help, submit suggestions, report bugs, or be with other users of my scripts")
menu.hyperlink(metaList, "吉特哈布源", "https://github.com/Jackzmc/lua-scripts", "View all my lua scripts on github")
...
if metaList then
    menu.divider(metaList, "-- Pop汉化（原作者团队介绍↓） --")
...
menu.action(menu.my_root(), "停止所有动作", {"stopself"}, "停止当前场景或动画", function(v)
    clear_anim_props()
...
menu.toggle(menu.my_root(), "立即清除", {"clearimmediately"}, "如果启用，将在激活新动画/场景时立即停止正在播放的动画/场景。 如果为关，您将顺利过渡到下一个动作.", function(on)
    lclearActionImmediately = on
end, clearActionImmediately)
menu.list_select(menu.my_root(), "行动目标", {"actiontarget"}, "将执行此操作的实体.\n0 = 只有你自己\n1 = 只有NPC\n2 = 你和NPC", { { "只有你自己" }, { "只有NPC" }, {"你和NPC"} }, 1, function(index)
    affectType = index - 1
end)
...
menu.divider(menu.my_root(), "项目")
local specialAnimationsMenu = menu.list(menu.my_root(), "特殊动画", {}, "可以使用道具的特殊动画")
menu.toggle(specialAnimationsMenu, "可控", {"animationcontrollable"}, "动画是否允许玩家控制?", onControllablePress, allowControl)
local animationsMenu = menu.list(menu.my_root(), "动画", {}, "您可以播放的动画列表")
...
local scenariosMenu = menu.list(menu.my_root(), "场景", {}, "您可以玩的场景列表\n某些场景仅适用于某些性别，例如 AA Coffee 仅适用于男性人物.")
for group, scenarios in pairs(SCENARIOS) do
    local submenu = menu.list(scenariosMenu, group, {}, "所有 " .. group .. " 场景")
...
local ambientSpeechMenu = menu.list(menu.my_root(), "环境语音", {}, "允许您对自己或其他NPC进行环境语音")
...
local speechType = menu.list(ambientSpeechMenu, "语音模式", {"speechmethods", "How is the line said"})
for _, pair in ipairs(SPEECH_PARAMS) do
    menu.action(speechType, pair[1], {}, pair[2], function(a)
        speechParam = pair[2]
    end)
end
local selfModelVoice = menu.list(ambientSpeechMenu, "自我语音模型", {"selfvoice", "使用什么模型播放自我环境语音\n仅当您当前的模型是(女)男时使用"})
menu.divider(selfModelVoice, "女性 儿科")
...
function download_animation_data()
    ...
    async_http.init("jackz.me", "/stand/resources/jackz_actions/animations.txt", function(result)
        ...
        util.toast(SCRIPT .. ": 无法自动下载动画数据文件。 请手动下载最新文件.")
        util.stop_script()
        loading = false
    end)
    async_http.dispatch()
