...
local function refresh_next_spin_time()
    ...
    if safe_playtime and safe_playtime ~= "00:00" then
        ...
        menus.next_spin_time.value = "已准备就绪!"
    end
end
...
menus.auto_spin = menu.toggle(menu.my_root(), "自动旋转", {}, "将传送到赌场，然后是高支出的老虎机。坐下后，它将自动旋转插槽，在输赢之间交替以避免被发现，直到达到每日限制。明天再回来，再次运行脚本以获取更多信息。", function(on)
    debug_log("Toggled auto-spin "..tostring(on))
    state.auto_spin = on
end)

menus.daily_winnings = menu.readonly(menu.my_root(), "每日奖金")
refresh_daily_winnings()
menus.next_spin_time = menu.readonly(menu.my_root(), "距离下一次旋转的时间")
refresh_next_spin_time()
...
local menu_options = menu.list(menu.my_root(), "选项", {}, "用于控制脚本行为方式的设置")
...
menu.toggle(menu_options, "自动提现", {}, "自动旋转完成后自动兑现筹码", function(on)
    config.auto_cash_out = on
end, config.auto_cash_out)
menu.toggle(menu_options, "调试模式", {}, "在 Stand/Log.txt 文件中包括其他详细信息，以帮助调试脚本问题。", function(on)
...
menu.action(menus.actions, "获取筹码", {}, "自动定位空置的高支出老虎机。", function()
    find_free_slot_machine()
end)
...
menu.action(menu_options, "清除旋转日志", {}, "删除所有历史记录。如果错误破坏了东西，这很有用，但要小心仍然遵守每天 5000 万美元的限制。", function()
    save_spin_log({})
    refresh_daily_winnings()
    refresh_next_spin_time()
end)
...
menus.script_meta = menu.list(menu.my_root(), "脚本信息")
menu.divider(menus.script_meta, "SlotBot")
menu.readonly(menus.script_meta, "版本", SCRIPT_VERSION)
menu.readonly(menus.script_meta, "翻译作者", "呆呆")
menu.hyperlink(menus.script_meta, "翻译lua群", "https://qm.qq.com/cgi-bin/qm/qr?k=RGZGWK_kEeWk3-pgyRcd6CbDQsnlbjyt&jump_from=webapi&authKey=KTreetFlIhfDf49MZN7MaFH1RXdtwU9Kcw0m/Wcx0r+fLXZKHL2d/VlIq6BVZ8Bv", "欢迎加入我们的大家庭")
